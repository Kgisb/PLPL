...
if view == "Funnel":
    ...
    # --- Funnel Pills (local selector) ---
    _fn_pill = st.radio("Funnel pills", ["None","Sib Detail"], index=0, horizontal=True, key="fn_local_pill")
    if _fn_pill == "Sib Detail":
        try:
...
def _render_funnel_sib_detail(df_f: pd.DataFrame):
    ...
    _df = df_f.copy()
    _df["_CREATE_DT"] = pd.to_datetime(_df[create_col], errors="coerce", dayfirst=True, infer_datetime_format=True)
    ...
    if scope == "Today":
        start = today.normalize(); end = today.normalize()
    elif scope == "Yesterday":
        start = (today - pd.Timedelta(days=1)).normalize(); end = start
    elif scope == "This Month":
        start = today.replace(day=1).normalize(); end = today.normalize()
    elif scope == "Last Month":
        ...
        if isinstance(c, tuple) and len(c)==2:
            start = pd.Timestamp(c[0]); end = pd.Timestamp(c[1])
        else:
            start = today.replace(day=1).normalize(); end = today.normalize()

    _win = _df[_df["_CREATE_DT"].between(start, end, inclusive="both")].copy()
    if _win.empty:
        st.warning("No rows in the selected window.", icon="⚠️")
        return

    pivot = _win.groupby([stage_col, sib_col]).size().rename("Count").reset_index()
    stage_order = pivot.groupby(stage_col)["Count"].sum().sort_values(ascending=False).index.tolist()
    sib_order   = pivot.groupby(sib_col)["Count"].sum().sort_values(ascending=False).index.tolist()
    ...
    total_row = tbl.sum(axis=0).to_frame().T
    total_row.index = ["Total"]
    table_out = pd.concat([tbl, total_row], axis=0)

    show_pct = st.checkbox("Show % within Deal Stage", value=False, key="sib_row_pct")
    if show_pct:
        pct = (tbl.div(tbl["Total"].replace(0, np.nan), axis=0)*100).round(1).fillna(0)
        pct["Total"] = 100.0
        pct_total_row = pct.mean(numeric_only=True).to_frame().T; pct_total_row.index=["Total"]
        st.dataframe(pd.concat([pct, pct_total_row], axis=0), use_container_width=True)
    else:
    ...
    st.altair_chart(
        alt.Chart(chart_df).mark_bar().encode(
            ...
            tooltip=[stage_col, sib_col, "Count"]
        ).properties(height=max(240, 28*len(stage_order)), width="container"),
        use_container_width=True
    )

    st.download_button("Download table (CSV)", data=table_out.reset_index().to_csv(index=False).encode("utf-8"),
                       file_name="sib_detail_by_stage.csv", mime="text/csv")