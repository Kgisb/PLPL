...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward", "Referral Pitched In", "Closed Lost Analysis", "Trial Trend", "Booking Analysis"],
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time" , "Sibling DS", "Sibling Details"],
    }
...
def _render_funnel_trial_trend(df_f, create_col_hint=None, counsellor_col_hint=None, country_col_hint=None, source_col_hint=None,
                               first_trial_col_hint=None, resched_col_hint=None, trial_done_col_hint=None, enrol_col_hint=None):
    import streamlit as st
    ...
    d["_TD"]   = _tt_to_dt(d[_done]) if _done else _pd.NaT
    ...
    c0,c1,c2,c3 = st.columns([1.0,1.0,1.4,1.6])
    ...
    with c1:
        ...
        if scope=="Today": start_d,end_d=today,today
        elif scope=="Yesterday": start_d=today-_timedelta(days=1); end_d=start_d
        elif scope=="This Month": start_d,end_d=_date(today.year,today.month,1), _date(today.year,today.month,1)+_timedelta(days=31)
        elif scope=="Last Month":
            first_this=_date(today.year,today.month,1); last_prev=first_this-_timedelta(days=1)
            start_d=_date(last_prev.year,last_prev.month,1); end_d=last_prev
        else:
            a,b=st.columns(2)
            with a: start_d=st.date_input("Start", value=_date(today.year,today.month,1), key="tt_start")
            with b: end_d=st.date_input("End", value=today, key="tt_end")
            if end_d<start_d: start_d,end_d=end_d,start_d
    with c2: gran = st.radio("Granularity", ["Daily","Monthly"], index=0, horizontal=True, key="tt_gran")
    with c3:
        dims=st.multiselect("Slice by", options=["Academic Counsellor","Country","JetLearn Deal Source"], default=["Academic Counsellor"], key="tt_dims")

    # Build events
    events=[]
    for _,row in d.iterrows():
        ac,cnt,src=row["_AC"],row["_CNT"],row["_SRC"]; cdate=row["_C"]
        trial_dates=set()
        if _pd.notna(row["_FT"]): trial_dates.add(_pd.Timestamp(row["_FT"]).normalize())
        if _pd.notna(row["_TR"]): trial_dates.add(_pd.Timestamp(row["_TR"]).normalize())
        for dt in sorted(trial_dates): events.append((dt,"Trial",ac,cnt,src,cdate))
        if _pd.notna(row["_TD"]): events.append((_pd.Timestamp(row["_TD"]).normalize(),"Trial Done",ac,cnt,src,cdate))
        if _pd.notna(row["_ENR"]): events.append((_pd.Timestamp(row["_ENR"]).normalize(),"Enrollment",ac,cnt,src,cdate))
        if _pd.notna(cdate): events.append((_pd.Timestamp(cdate).normalize(),"Lead",ac,cnt,src,cdate))
    if not events:
        import streamlit as st
    ...
    if mode=="MTD" and _create:
        in_win = in_win & _pd.to_datetime(ev["_C"]).dt.date.between(start_d,end_d)
    ...
    def _resolve(vals, all_vals): return all_vals if ("All" in vals or not vals) else vals
    ...
    if gran=="Daily":
        grp=["_day"]+dim_cols+["Metric"]; ser=ev.groupby(grp, dropna=False).size().rename("Count").reset_index(); x_enc=_alt.X("_day:T", title=None)
    else:
        grp=["_mon"]+dim_cols+["Metric"]; ser=ev.groupby(grp, dropna=False).size().rename("Count").reset_index(); x_enc=_alt.X("_mon:T", title=None)

    chart = st.radio("Chart", ["Stacked Bar","Horizontal Bar","Line"], index=0, horizontal=True, key="tt_chart")
    if chart=="Stacked Bar":
        ch=_alt.Chart(ser).mark_bar().encode(x=x_enc, y=_alt.Y("Count:Q"), color=_alt.Color("Metric:N", title="Metric"),
                                             tooltip=[_alt.Tooltip("Metric:N"), _alt.Tooltip("Count:Q")]).properties(height=320)
    elif chart=="Horizontal Bar":
        y_enc = _alt.Y("_day:T", title=None) if gran=="Daily" else _alt.Y("_mon:T", title=None)
        ch=_alt.Chart(ser).mark_bar().encode(y=y_enc, x=_alt.X("Count:Q"), color=_alt.Color("Metric:N", title="Metric"),
                                             tooltip=[_alt.Tooltip("Metric:N"), _alt.Tooltip("Count:Q")]).properties(height=320)
    else:
        ch=_alt.Chart(ser).mark_line(point=True).encode(x=x_enc, y=_alt.Y("Count:Q"), color=_alt.Color("Metric:N", title="Metric"),
                                                       tooltip=[_alt.Tooltip("Metric:N"), _alt.Tooltip("Count:Q")]).properties(height=320)
    st.altair_chart(ch, use_container_width=True)
    ...
    piv = ser.pivot_table(index=time_col, columns="Metric", values="Count", aggfunc="sum", fill_value=0).reset_index()
    if den in piv.columns and num in piv.columns:
        piv["Rate"]=_np.where(piv[den]>0, piv[num]/piv[den], _np.nan)
        ch_rate=_alt.Chart(piv).mark_line(point=True).encode(
            x=_alt.X(f"{time_col}:T", title=None), y=_alt.Y("Rate:Q", axis=_alt.Axis(format="%"), title=f"{num} / {den}"),
            tooltip=[_alt.Tooltip(f"{time_col}:T", title="Time"), _alt.Tooltip(den, title=den), _alt.Tooltip(num, title=num), _alt.Tooltip("Rate:Q", format=".2%")]
        ).properties(height=300)
        st.altair_chart(ch_rate, use_container_width=True)
        pretty_rate=piv.rename(columns={time_col: ("Date" if gran=="Daily" else "Month")})
...
# ---- Router hooks for new pills ----
try:
    if view == "Closed Lost Analysis":
        _render_funnel_closed_lost_analysis(
            df,
            counsellor_col_hint=locals().get("counsellor_col", None),
            country_col_hint=locals().get("country_col", None),
            source_col_hint=locals().get("source_col", None),
    ...
    if view == "Trial Trend":
        _render_funnel_trial_trend(
            ...
            country_col_hint=locals().get("country_col", None),
            source_col_hint=locals().get("source_col", None),
            first_trial_col_hint=locals().get("first_cal_sched_col", None) or locals().get("trial_s_col", None),
            resched_col_hint=locals().get("cal_resched_col", None) or locals().get("trial_r_col", None),
            trial_done_col_hint=locals().get("cal_done_col", None),
            enrol_col_hint=None
        )
except Exception as _e:
    import streamlit as st
    st.error(f"New pill rendering failed: {type(_e).__name__}: {_e}")





# --- Marketing → Sibling DS ---
# --- Marketing → Sibling DS ---
def _render_marketing_sibling_ds(
    df_f: pd.DataFrame,
    sibling_col_hint: str = None,
    create_col_hint: str = None,
    payment_col_hint: str = None,
    first_trial_col_hint: str = None,
    resched_col_hint: str = None,
    trial_done_col_hint: str = None,
    cal_done_col_hint: str = None,
    referral_intent_col_hint: str = None,
):
    import streamlit as st
    import pandas as pd
    import numpy as np
    ...
    st.subheader("👨‍👧 Sibling DS — split by values")

    # ---- Resolve columns ----
    def _resolve(col_hint, candidates):
        # include stripped/lowered loose matching
        if col_hint and col_hint in df_f.columns:
            return col_hint
        cols_norm = {c.strip().lower(): c for c in df_f.columns}
        for c in candidates:
            if c in df_f.columns:
                return c
            if c.strip().lower() in cols_norm:
                return cols_norm[c.strip().lower()]
        try:
            return find_col(df_f, candidates)
        except Exception:
            return None

    sibling_col = _resolve(sibling_col_hint, ["Sibling Deal","Sibling D","Sibling","Sibling_DS","Siblings"])
    create_col = _resolve(create_col_hint, ["Create Date","Create_Date","Created At","Deal Create Date"])
    payment_col = _resolve(payment_col_hint, ["Payment Received Date","Payment Received Date ","Enrollment Date","Payment Date"])
    # Booking / First trial
    first_trial_col = _resolve(first_trial_col_hint, [
        "First Calibration Scheduled Date",
        "[Trigger] - Calibration Booking Date",
        "First Trial Scheduled Date",
        "Trial Scheduled Date"
    ])
    resched_col = _resolve(resched_col_hint, ["Calibration Rescheduled Date","Trial Rescheduled Date"])
    # Trial/Cal done
    trial_done_col = _resolve(trial_done_col_hint, ["Trial Done Date","First Trial Done Date"])
    cal_done_col = _resolve(cal_done_col_hint, ["Calibration Done Date"])
    ris_col = _resolve(referral_intent_col_hint, ["Referral Intent Source","Referral Intent","RIS"])

    if sibling_col is None:
        st.error("Sibling column not found. Expected one of: Sibling Deal / Sibling / Siblings")
        return
    if create_col is None:
        st.error("Create Date column not found. This view needs Create Date to anchor windows.")
        return

    # Coerce dates (dayfirst tolerant)
    def _to_dt(s):
        try:
            return pd.to_datetime(s, errors="coerce", dayfirst=True, infer_datetime_format=True)
        except Exception:
            return pd.to_datetime(s, errors="coerce")
    for c in [create_col, payment_col, first_trial_col, resched_col, trial_done_col, cal_done_col]:
        if c and c in df_f.columns:
            df_f[c] = _to_dt(df_f[c])

    # ---- Controls ----
    cols = st.columns([1.2,1.2,1,1.6,1])
    with cols[0]:
        mode = st.radio("Mode", ["MTD","Cohort"], index=0, horizontal=True, key="sibds_mode")
    with cols[1]:
        preset = st.selectbox("Window", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, key="sibds_preset")
    with cols[2]:
        gran = st.selectbox("Granularity", ["Day","Week","Month"], index=0, key="sibds_gran")
    with cols[3]:
        metric = st.radio(
            "Metric",
            ["Total","Deals","Enrollments","Biggest Stage","Calibration Done","First Calibration","Trial Scheduled","Trial Rescheduled","Sales Generated (RIS)"],
            index=0, key="sibds_metric")
    with cols[4]:
        ris_only = st.toggle("RIS only", value=False, key="sibds_ris_only")

    # Custom window
    if preset == "Custom":
        c1, c2 = st.columns(2)
        with c1:
            start_d = st.date_input("Start", value=pd.Timestamp.today().normalize().replace(day=1), key="sibds_start")
        with c2:
            end_d = st.date_input("End", value=pd.Timestamp.today().normalize(), key="sibds_end")
    else:
        today = pd.Timestamp.today().normalize()
        if preset == "Today":
            start_d = end_d = today
        elif preset == "Yesterday":
            start_d = end_d = today - pd.Timedelta(days=1)
        elif preset == "This Month":
    ...
    end_d = pd.Timestamp(end_d) + pd.Timedelta(hours=23, minutes=59, seconds=59)

    # ---- Event date by metric ----
    def _event_col_for_metric(metric_name):
        if metric_name in ["Total","Deals"]:
            return create_col
        if metric_name == "Enrollments":
            return payment_col
        if metric_name == "Calibration Done":
            return cal_done_col
        if metric_name == "First Calibration":
            return first_trial_col
        if metric_name == "Trial Scheduled":
            return first_trial_col
        if metric_name == "Trial Rescheduled":
            return resched_col
        if metric_name == "Sales Generated (RIS)":
            return create_col
        return create_col

    ev_col = _event_col_for_metric(metric)

    # ---- Base filtering ----
    base = df_f.copy()
    # filter by RIS if needed
    if ris_only and ris_col and ris_col in base.columns:
        base = base[(base[ris_col].astype(str).str.strip().str.lower() == "sales generated")]
    if metric == "Sales Generated (RIS)":
        if ris_col and ris_col in base.columns:
            base = base[(base[ris_col].astype(str).str.strip().str.lower() == "sales generated")]

    # for metrics that require an event date, drop NA event rows
    needs_event = metric not in ["Biggest Stage","Total"]
    if ev_col and needs_event:
        base = base[ev_col.notna()]

    # Windowing
    if ev_col is None:
        st.warning("Required date column for the selected metric is missing; showing empty result.")
        base = base.iloc[0:0]
    if mode == "MTD":
        mask = base[create_col].between(start_d, end_d, inclusive="both")
        if ev_col is not None:
            mask = mask & base[ev_col].between(start_d, end_d, inclusive="both")
        base = base[mask]
    else:
        col = ev_col if ev_col is not None else create_col
        base = base[base[col].between(start_d, end_d, inclusive="both")]

    # ---- Compute table ----
    if metric == "Biggest Stage":
        stage_col = _resolve(None, ["Deal Stage"])
        if stage_col is None:
            st.error("Deal Stage column not found for 'Biggest Stage'.")
            return
        agg = (base.groupby([sibling_col, stage_col], dropna=False)
                    .size().reset_index(name="Count"))
        top_stage = (agg.sort_values(["Count"], ascending=False)
                        .groupby(sibling_col, as_index=False)
                        .first()
                        .rename(columns={stage_col:"Top Stage","Count":"Top Count"}))
        table = (agg.groupby(sibling_col, dropna=False)["Count"].sum().reset_index(name="Total"))
        table = table.merge(top_stage[[sibling_col,"Top Stage","Top Count"]], on=sibling_col, how="left")
        table["% Share"] = (table["Total"] / max(table["Total"].sum(), 1)) * 100.0
        table = table.sort_values("Total", ascending=False)
    else:
        table = (base.groupby(sibling_col, dropna=False).size().reset_index(name="Count"))
        table["% Share"] = (table["Count"] / max(table["Count"].sum(), 1)) * 100.0
    ...
    # Time series (only when metric has event col)
    if ev_col is not None and base.shape[0] > 0:
        st.markdown("### Trend")
        grp = base.copy()
        if gran == "Day":
            grp["Date"] = base[ev_col].dt.to_period("D").apply(lambda p: p.start_time)
        elif gran == "Week":
            grp["Date"] = base[ev_col].dt.to_period("W").apply(lambda p: p.start_time)
        else:
            grp["Date"] = base[ev_col].dt.to_period("M").apply(lambda p: p.start_time)
        grp = grp.groupby([sibling_col, "Date"]).size().reset_index(name="Count")

        if len(grp):
            ch = alt.Chart(grp).mark_bar().encode(
                x="Date:T",
                y="Count:Q",
                color=alt.Color(f"{sibling_col}:N", legend=alt.Legend(title="Sibling")),
                tooltip=[alt.Tooltip("Date:T"), alt.Tooltip(f"{sibling_col}:N"), alt.Tooltip("Count:Q")]
            ).properties(height=300)
            st.altair_chart(ch, use_container_width=True)
        else:
            st.info("No rows in the selected window for a time series.")

    # ---- Download ----
    csv = table.to_csv(index=False).encode("utf-8")
    st.download_button("⬇️ Download CSV", data=csv, file_name="sibling_ds_summary.csv", mime="text/csv")



# --- Marketing → Sibling Details ---
def _render_marketing_sibling_details(
    df_f: pd.DataFrame,
    sibling_col_hint: str = "Sibling Deal",
    stage_col_hint: str = None,
    create_col_hint: str = None,
):
    import streamlit as st
    import pandas as pd
    ...
    st.subheader("👨‍👧 Sibling Details — Deal Stage distribution")

    # --- Resolve columns (prefer exact "Sibling Deal" as requested) ---
    def _resolve(col_hint, candidates):
        if col_hint and col_hint in df_f.columns:
            return col_hint
        # tolerant matching
        norm = {c.strip().lower(): c for c in df_f.columns}
        for c in candidates:
            if c in df_f.columns:
                return c
            lc = c.strip().lower()
    ...
    sibling_col = _resolve(sibling_col_hint, ["Sibling Deal","Siblings","Sibling","Sibling_DS"])
    stage_col = _resolve(stage_col_hint, ["Deal Stage","Stage"])
    create_col = _resolve(create_col_hint, ["Create Date","Create_Date","Created At","Deal Create Date"])
    ris_col = _resolve(None, ["Referral Intent Source","Referral Intent","RIS"])

    if sibling_col is None:
        st.error("Sibling column not found (expected 'Sibling Deal').")
        return
    if stage_col is None:
        st.error("Deal Stage column not found.")
        return
    if create_col is None:
        st.error("Create Date column not found.")
        return

    # Coerce create date
    try:
        df_f[create_col] = pd.to_datetime(df_f[create_col], errors="coerce", dayfirst=True, infer_datetime_format=True)
    except Exception:
        df_f[create_col] = pd.to_datetime(df_f[create_col], errors="coerce")

    # --- Controls ---
    c = st.columns([1.1,1.4,1.4,1.1])
    with c[0]:
        mode = st.radio("Mode", ["MTD","Cohort"], index=0, horizontal=True, key="sibdet_mode")
    with c[1]:
        preset = st.selectbox("Window", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, key="sibdet_preset")
    with c[2]:
        _ = st.empty()  # spacer
    with c[3]:
        view_kind = st.radio("View", ["Table","Stacked Bar"], index=0, horizontal=True, key="sibdet_view")
    ris_only = st.toggle("RIS only (Sales Generated)", value=False, key="sibdet_ris")

    # Custom dates
    if preset == "Custom":
        d1, d2 = st.columns(2)
        with d1:
            start_d = st.date_input("Start", value=pd.Timestamp.today().normalize().replace(day=1), key="sibdet_start")
        with d2:
            end_d = st.date_input("End", value=pd.Timestamp.today().normalize(), key="sibdet_end")
    else:
        today = pd.Timestamp.today().normalize()
        if preset == "Today":
            start_d = end_d = today
    ...
    # --- Filter window (anchor = Create Date as per spec) ---
    base = df_f.copy()
    # RIS filter (Sales Generated): ON -> only SG, OFF -> only non-SG
    if ('ris_only' in locals() or 'ris_only' in globals()):
        if (ris_col is not None) and (ris_col in base.columns):
            _ris_norm = base[ris_col].astype(str).str.strip().str.lower()
            if ris_only:
                base = base[_ris_norm == "sales generated"]
            else:
                base = base[_ris_norm != "sales generated"]
    base = base[base[create_col].notna()]
    if mode == "MTD":
        # MTD equals Cohort here because only Create Date is anchor for this tab; still honor the definition
        mask = base[create_col].between(start_d, end_d, inclusive="both")
        base = base[mask]
    else:
        # Cohort: same anchor (Create Date)
        base = base[base[create_col].between(start_d, end_d, inclusive="both")]

    # --- Build pivot table: Sibling x Deal Stage counts ---
    if base.empty:
        st.info("No rows in the selected window.")
        return

    # Clean sibling text a bit
    sib = base[sibling_col].astype(str).str.strip().replace({"": "Blank"})
    stage = base[stage_col].astype(str).str.strip().replace({"": "Blank"})

    pt = (pd.crosstab(sib, stage, dropna=False).rename_axis(index="Sibling", columns="Deal Stage").reset_index())
    pt["Total"] = pt.drop(columns=["Sibling"]).sum(axis=1)
    # percentage share of each sibling across all siblings
    total_all = max(pt["Total"].sum(), 1)
    pt["% of All"] = (pt["Total"] / total_all * 100).round(2)
...
if ('ris_only' in locals() or 'ris_only' in globals()):
    if ris_only:
        st.caption("Filter: Referral Intent Source = **Sales Generated**")
    else:
        st.caption("Filter: Referral Intent Source ≠ **Sales Generated** (including blanks)")
    st.dataframe(pt, use_container_width=True)

    # Optional stacked bar
    if view_kind == "Stacked Bar":
        df_long = pt.melt(id_vars=["Sibling","Total","% of All"], var_name="Deal Stage", value_name="Count")
        df_long = df_long[df_long["Deal Stage"].ne("% of All")]
        df_long = df_long[df_long["Deal Stage"].ne("Total")]
        if df_long["Count"].sum() > 0:
            ch = alt.Chart(df_long).mark_bar().encode(
                x=alt.X("Sibling:N", sort="-y"),
                y="Count:Q",
                color=alt.Color("Deal Stage:N", legend=alt.Legend(title="Deal Stage")),
                tooltip=["Sibling","Deal Stage","Count"]
            ).properties(height=320)
            st.altair_chart(ch, use_container_width=True)
        else:
            st.info("No counts to chart for the current window.")

    # Download
    csv = pt.to_csv(index=False).encode("utf-8")
    st.download_button("⬇️ Download CSV", data=csv, file_name="sibling_details_deal_stages.csv", mime="text/csv")


# --- Route: Sibling DS ---
try:
    if view == "Sibling DS":
        _render_marketing_sibling_ds(
            df,
            sibling_col_hint=locals().get("sibling_col", None),
            create_col_hint=locals().get("create_col", None),
            payment_col_hint=locals().get("payment_col", None),
            first_trial_col_hint=locals().get("first_cal_sched_col", None) or locals().get("trial_s_col", None),
            resched_col_hint=locals().get("cal_resched_col", None) or locals().get("trial_r_col", None),
            trial_done_col_hint=locals().get("trial_done_col", None),
            cal_done_col_hint=locals().get("cal_done_col", None),
            referral_intent_col_hint=locals().get("ris_col", None) or locals().get("referral_intent_col", None)
        )
except Exception:
    pass


# --- Route: Sibling Details ---
try:
    if view == "Sibling Details":
        _render_marketing_sibling_details(
            df,
            sibling_col_hint="Sibling Deal",
            stage_col_hint=locals().get("deal_stage_col", None) or "Deal Stage",
            create_col_hint=locals().get("create_col", None) or "Create Date",
        )
except Exception as _e:
    import streamlit as st
    st.error(f"Sibling Details failed: {type(_e).__name__}: {_e}")
