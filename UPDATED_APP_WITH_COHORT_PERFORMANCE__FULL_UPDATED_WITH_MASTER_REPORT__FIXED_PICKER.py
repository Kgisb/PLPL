...
def render_master_report(_df_host):
    ...
    def _apply_mode(_df_in):
        if _mode == "MTD":
            _first_m = _pd_mr.Timestamp(_date_mr.today().replace(day=1))
            return _df_in, _first_m, _pd_mr.Timestamp(_date_mr.today())
        else:
            if not _COLS.get("create"):
