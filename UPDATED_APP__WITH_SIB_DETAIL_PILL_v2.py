...
# =========================
    # --- Pill: Sib Detail (Sibling Deal × Deal Stage) ---
    try:
        _render_funnel_sib_detail(df)
    except Exception as e:
        st.warning(f"Sib Detail failed: {e}")
...
# ---- Date & Time & Duration parsers ----
        # --- Pill: Sib Detail (Sibling Deal × Deal Stage)
        try:
            _render_funnel_sib_detail(df_f)
        except Exception as e:
            st.warning(f"Sib Detail failed: {e}")
...
# ---------------------------
# Funnel & Movement — Sib Detail (Sibling Deal × Deal Stage, Create Date-scoped)
def _render_funnel_sib_detail(df_f: pd.DataFrame):
    import streamlit as st
    import pandas as pd
    import numpy as np
    import altair as alt
    ...
    st.markdown("### Sib Detail — Sibling Deal × Deal Stage (Create Date scoped)")

    # Resolve columns safely
    def find_col(_df, candidates):
        for c in candidates:
            if c in _df.columns:
                return c
        # case-insensitive
        cl = {c.lower(): c for c in _df.columns}
        for c in candidates:
            if c.lower() in cl:
                return cl[c.lower()]
        return None

    create_col = find_col(df_f, ["Create Date","Created Date","Deal Create Date","Created On"])
    stage_col  = find_col(df_f, ["Deal Stage","Stage"])
    sib_col    = find_col(df_f, ["Sibling Deal","Sibling deal","Sibling"])

    if not create_col or not stage_col or not sib_col:
        st.info("Required columns not found (need ‘Create Date’, ‘Deal Stage’, and ‘Sibling Deal’).", icon="ℹ️")
        return

    # Parse dates (day-first as your data uses DD-MM-YYYY in places)
    _dt = pd.to_datetime(df_f[create_col], errors="coerce", dayfirst=True, infer_datetime_format=True)
    _df = df_f.copy()
    _df["_CREATE_DT"] = _dt

    # Filter out invalid deals consistently with the rest of the app
    if stage_col in _df.columns:
        _df = _df[_df[stage_col].astype(str).str.strip().str.lower() != "1.2 invalid deal(s)"]

    # Quick date scopes
    scope = st.selectbox("Date scope (Create Date)", ["Today","Yesterday","This Month","Last Month","Custom"], index=2)
    today = pd.Timestamp(date.today())
    if scope == "Today":
        start = today.normalize()
        end = today.normalize()
    elif scope == "Yesterday":
        start = (today - pd.Timedelta(days=1)).normalize()
        end = start
    elif scope == "This Month":
        start = today.replace(day=1).normalize()
        end = today.normalize()
    elif scope == "Last Month":
        first_this = today.replace(day=1).normalize()
        end = first_this - pd.Timedelta(days=1)
        start = end.replace(day=1).normalize()
    else:
        c = st.date_input("Custom range (Create Date)", (today.replace(day=1).date(), today.date()))
        if isinstance(c, tuple) and len(c)==2:
            start = pd.Timestamp(c[0])
            end = pd.Timestamp(c[1])
        else:
            start = today.replace(day=1).normalize()
            end = today.normalize()

    # Apply window
    mask = _df["_CREATE_DT"].between(start, end, inclusive="both")
    _win = _df.loc[mask].copy()

    if _win.empty:
        st.warning("No rows in the selected window.", icon="⚠️")
        return

    # Build pivot: Deal Stage × Sibling Deal (counts)
    pivot = (
        _win.groupby([stage_col, sib_col]).size().rename("Count").reset_index()
    )
    # Order stages by total volume
    stage_order = (
        pivot.groupby(stage_col)["Count"].sum()
        .sort_values(ascending=False)
        .index.tolist()
    )
    sib_order = (
        pivot.groupby(sib_col)["Count"].sum()
        .sort_values(ascending=False)
        .index.tolist()
    )

    # Table view with totals and optional row percentages
    show_pct = st.checkbox("Show % within Deal Stage", value=False, key="sib_row_pct")
    # Build wide table
    tbl = pivot.pivot_table(index=stage_col, columns=sib_col, values="Count", aggfunc="sum", fill_value=0)
    tbl = tbl.reindex(index=stage_order, columns=sib_order)
    tbl["Total"] = tbl.sum(axis=1)
    total_row = pd.DataFrame([tbl.sum(axis=0)], index=["Total"])
    table_out = pd.concat([tbl, total_row], axis=0)

    if show_pct:
        pct_tbl = (tbl.div(tbl["Total"].replace(0, np.nan), axis=0) * 100).round(1).fillna(0)
        pct_tbl["Total"] = 100.0
        pct_total_row = pd.DataFrame([pct_tbl.mean(numeric_only=True)], index=["Total"])  # not strict but indicative
        pct_out = pd.concat([pct_tbl, pct_total_row], axis=0)
        st.dataframe(pct_out, use_container_width=True)
    else:
        st.dataframe(table_out, use_container_width=True)

    # Chart — stacked bar by Deal Stage
    st.caption("Distribution by Deal Stage (stacked by Sibling Deal)")
    chart_df = pivot.copy()
    chart_df[stage_col] = pd.Categorical(chart_df[stage_col], categories=stage_order, ordered=True)
    chart_df[sib_col] = pd.Categorical(chart_df[sib_col], categories=sib_order, ordered=True)
    ...
    ch = (
        ...
        .mark_bar()
        .encode(
            y=alt.Y(f"{stage_col}:N", sort=stage_order, title="Deal Stage"),
            x=alt.X("Count:Q", title="Count"),
            color=alt.Color(f"{sib_col}:N", legend=alt.Legend(title="Sibling Deal")),
            tooltip=[alt.Tooltip(f"{stage_col}:N"), alt.Tooltip(f"{sib_col}:N"), alt.Tooltip("Count:Q")]
        )
        .properties(height=max(240, 28*len(stage_order)), width="container")
    )
    st.altair_chart(ch, use_container_width=True)

    # Download aggregated CSV
    csv = table_out.reset_index().to_csv(index=False).encode("utf-8")
    st.download_button("Download table (CSV)", data=csv, file_name="sib_detail_by_stage.csv", mime="text/csv")
