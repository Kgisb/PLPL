...
def _jl_resolve_col(df, preferred, candidates):
    if isinstance(preferred, str) and preferred in df.columns:
        return preferred
    for c in candidates:
        if c in df.columns:
...
def _jl_classify_interval(row, cols):
...
def _render_funnel_closed_lost_analysis(
    df_f,
    ...
    trial_s_col_hint: str | None = None,
    trial_r_col_hint: str | None = None,
    cal_done_col_hint: str | None = None,
    closed_lost_col_hint: str | None = None,
):
    import streamlit as st

    st.subheader("Funnel & Movement — Closed Lost Analysis")

    _counsellor = _jl_resolve_col(df_f, counsellor_col_hint, ["Student/Academic Counsellor","Academic Counsellor","Counsellor","Counselor","Deal Owner"])
    _country    = _jl_resolve_col(df_f, country_col_hint,    ["Country","Country Name"])
    _source     = _jl_resolve_col(df_f, source_col_hint,     ["JetLearn Deal Source","Deal Source","Source","Original source"])
    _create     = _jl_resolve_col(df_f, create_col_hint,     ["Create Date","Created Date","Deal Create Date","CreateDate","Created On","Creation Date"])
    _trial_s    = _jl_resolve_col(df_f, trial_s_col_hint,    ["Trial Scheduled Date","Trial Schedule Date","Trial Booking Date","First Calibration Scheduled Date"])
    _trial_r    = _jl_resolve_col(df_f, trial_r_col_hint,    ["Trial Rescheduled Date","Trial Re-scheduled Date","Calibration Rescheduled Date"])
    _cal_done   = _jl_resolve_col(df_f, cal_done_col_hint,   ["Calibration Done Date","Calibration Completed Date","Trial Done Date"])
    _cl_date    = _jl_resolve_col(df_f, closed_lost_col_hint,["[Deal Stage] - Closed Lost Trigger Date","Closed Lost Trigger Date","Closed Lost Date","Closed-Lost Trigger Date"])
    ...
    d = df_f.copy()
    d["_CL"]  = _jl_to_dt(d[_cl_date])
    d["_C"]   = _jl_to_dt(d[_create])
    d["_TS"]  = _jl_to_dt(d[_trial_s]) if _trial_s else _pd.NaT
    d["_TR"]  = _jl_to_dt(d[_trial_r]) if _trial_r else _pd.NaT
    d["_CD"]  = _jl_to_dt(d[_cal_done]) if _cal_done else _pd.NaT

    d["_AC"]   = _jl_norm_text(d[_counsellor]) if _counsellor else _pd.Series(["Unknown"]*len(d))
    d["_CNT"]  = _jl_norm_text(d[_country]) if _country else _pd.Series(["Unknown"]*len(d))
    d["_SRC"]  = _jl_norm_text(d[_source]) if _source else _pd.Series(["Unknown"]*len(d))
    ...
    dd["_interval"], dd["_days_to_loss"] = zip(*dd.apply(lambda r: _jl_classify_interval(r, cols_map), axis=1))
...
# Ensure pill in menu at runtime if dict wasn't found earlier
try:
...
try:
    if view == "Closed Lost Analysis":
        _csl_hint = locals().get("counsellor_col", None)
        _cnt_hint = locals().get("country_col", None)
        _src_hint = locals().get("source_col", None)
        _cre_hint = locals().get("create_col", None)
        _ts_hint  = locals().get("first_cal_sched_col", None) or locals().get("trial_s_col", None)
        _tr_hint  = locals().get("cal_resched_col", None) or locals().get("trial_r_col", None)
        _cd_hint  = locals().get("cal_done_col", None)
        _cl_hint  = None
        _render_funnel_closed_lost_analysis(
            df,
            counsellor_col_hint=_csl_hint,
            country_col_hint=_cnt_hint,
            source_col_hint=_src_hint,
            create_col_hint=_cre_hint,
            trial_s_col_hint=_ts_hint,
            trial_r_col_hint=_tr_hint,
            cal_done_col_hint=_cd_hint,
            closed_lost_col_hint=_cl_hint,
        )
except Exception as _e:
    import streamlit as st
...
def _render_funnel_booking_analysis(
    df_f,
    trigger_book_col_hint: str | None = None,
    cal_slot_col_hint: str | None = None,
    first_cal_sched_col_hint: str | None = None,
    counsellor_col_hint: str | None = None,
    country_col_hint: str | None = None,
    source_col_hint: str | None = None,
):
    ...
    # booking_type: Pre-book if slot has value, else Self book (requires first date present)
    ...
    with c3:
        dims = st.multiselect("Slice by", options=["Academic Counsellor","Country","JetLearn Deal Source","Booking Type"],
    ...
    in_win = d["_TRIG"].dt.date.between(start_d, end_d)
    ...
    # Map dims to columns
    def _map_dim(x):
        return {"Academic Counsellor":"_AC", "Country":"_CNT", "JetLearn Deal Source":"_SRC", "Booking Type":"_BKTYPE"}.get(x)
    ...
    pretty = series.rename(columns={
        "_day":"Date","_mon":"Month",
        "_AC":"Academic Counsellor","_CNT":"Country","_SRC":"JetLearn Deal Source","_BKTYPE":"Booking Type"
    })
    st.dataframe(pretty, use_container_width=True, hide_index=True)
    st.download_button(
        "Download CSV — Booking Analysis Trend",
        data=pretty.to_csv(index=False).encode("utf-8"),
        file_name="booking_analysis_trend.csv",
        mime="text/csv",
        key="bk_dl_tbl"
    )

    # ---- Comparison: Booking Trigger vs Trial Scheduled (Daily/Monthly)
    ...
    if do_compare:
        # Build Trial Scheduled series using the same date window and the same AC/Country/Source filters.
        ts = d.copy()
        ts = ts[ts["_FIRST"].notna()].copy()
        ts_in = ts["_FIRST"].dt.date.between(start_d, end_d)
        ts = ts.loc[ts_in].copy()

        # Apply the same AC/Country/Source filters (Booking Type does not apply to trial series)
        ts["_AC"] = ts["_AC"].fillna("Unknown")
        ts["_CNT"] = ts["_CNT"].fillna("Unknown")
        ts["_SRC"] = ts["_SRC"].fillna("Unknown")
        ...
        if gran == "Daily":
            ...
            # Normalize Booking series to the same x column
            if "_day" in bk_series.columns:
                bk_series["_x"] = bk_series["_day"]
            else:
                # When granularity switched mid-run, recompute quick booking daily totals
            ...
            bk_series = series.copy()
            if "_mon" in bk_series.columns:
                bk_series["_x"] = bk_series["_mon"]
            else:
        ...
        bk_series_slim = bk_series[["_x","Count"]].copy()
        bk_series_slim["Series"] = "Booking Trigger"

        comp = _pd.concat([bk_series_slim, ts_series[["_x","Count","Series"]]], ignore_index=True)
        comp = comp.sort_values(["_x","Series"]).reset_index(drop=True)

        # Draw comparison as lines
        ch_cmp = (
            _alt.Chart(comp)
            .mark_line(point=True)
            .encode(
                x=_alt.X("_x:T", title=None),
                y=_alt.Y("Count:Q"),
                color=_alt.Color("Series:N", title="Series"),
                tooltip=[_alt.Tooltip("_x:T", title="Date/Month"),
        ...
        st.download_button(
            "Download CSV — Booking vs Trial Scheduled",
            data=pretty_cmp.to_csv(index=False).encode("utf-8"),
            file_name="booking_vs_trial_scheduled.csv",
            mime="text/csv",
            key="bk_dl_cmp"
        )
...
try:
    if view == "Booking Analysis":
        _render_funnel_booking_analysis(
            df,
