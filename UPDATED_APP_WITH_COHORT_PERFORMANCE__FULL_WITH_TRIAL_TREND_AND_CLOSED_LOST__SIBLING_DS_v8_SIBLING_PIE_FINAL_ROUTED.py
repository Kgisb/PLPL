...
def _render_funnel_sibling_pie(df, sibling_col_hint="Sibling Deal", stage_col_hint="Deal Stage", ris_col_hint="Referral Intent Source"):
    ...
    date_col = "Create Date" if "Create Date" in df.columns else None

    if not sibling_col or not stage_col or not date_col:
        st.warning("Sibling Deal, Deal Stage or Create Date column missing."); return

    # RIS filter
    if ris_col:
        vals = sorted(df[ris_col].dropna().unique().tolist())
        selected = st.multiselect("Referral Intent Source", ["All"] + vals, default=["All"], key="siblingpie_ris")
        if "All" not in selected:
            df = df[df[ris_col].isin(selected)]

    # Handle blanks
    df[sibling_col] = df[sibling_col].fillna("(Blank)")
    df[stage_col] = df[stage_col].fillna("(Unknown)")

    # Aggregation
    grouped = df.groupby([stage_col, sibling_col]).size().reset_index(name="Count")
    ...
    # KPIs
    st.metric("Total Deals in scope", len(df))
    st.metric("Sibling deals", df[sibling_col].notna().sum())
    st.metric("Distinct sibling values", df[sibling_col].nunique())

    # Chart
    df_long = grouped.copy()
    if not df_long.empty:
        chart = alt.Chart(df_long).mark_bar().encode(
            x=alt.X(f"{sibling_col}:N", sort="-y"),
            y="Count:Q",
...
try:
    if view == "Sibling Pie":
        _render_funnel_sibling_pie(df)
except Exception as _e:
    import streamlit as st
    st.error(f"Sibling Pie failed: {type(_e).__name__}: {_e}")
