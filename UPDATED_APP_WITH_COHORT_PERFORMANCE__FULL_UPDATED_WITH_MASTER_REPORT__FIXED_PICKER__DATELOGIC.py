...
# ====================== BEGIN: Master report (INLINE, corrected columns) ======================
import pandas as _pd_mr
import numpy as _np_mr
import altair as _alt_mr
import streamlit as _st_mr
from datetime import date as _date_mr, timedelta as _timedelta_mr

_MR_KEY = "Funnel&Movement_master_report_"

def _mr_find_col(_df, _cands):
    for _c in _cands:
...
def _mr_to_dt(_s):
    return _pd_mr.to_datetime(_s, errors="coerce", dayfirst=True)

def _mr_freq(_g):
    return {"Day":"D","Week":"W","Month":"M","Quarter":"Q","Year":"Y"}[_g]

def _mr_preset(_preset, _today=None):
    if _today is None: _today = _date_mr.today()
    if _preset=="Today":
        _start=_today; _end=_today
    elif _preset=="Yesterday":
        _start=_today-_timedelta_mr(days=1); _end=_today-_timedelta_mr(days=1)
    elif _preset=="This Month":
        _start=_today.replace(day=1); _end=_today
    elif _preset=="Last Month":
        _first_this=_today.replace(day=1)
        _last_end=_first_this-_timedelta_mr(days=1)
        _start=_last_end.replace(day=1); _end=_last_end
    else:
        _start=_today.replace(day=1); _end=_today
    return _pd_mr.to_datetime(_start), _pd_mr.to_datetime(_end)

def render_master_report(_df_host):
    _st_mr.markdown("## Master report")
    if _df_host is None or len(_df_host)==0:
        _st_mr.info("No data available in current filter context.")
        return

    # Try reuse top-level COLS if exists; else resolve locally
    _COLS = globals().get("COLS", {})
    if not _COLS:
        _COLS = {}
        _COLS["id"] = _mr_find_col(_df_host, ["Record ID","Deal ID","ID","_id"]) or "_synthetic_id"
        if "_synthetic_id" not in _df_host.columns:
            _df_host = _df_host.copy()
            _df_host["_synthetic_id"] = _np_mr.arange(len(_df_host)) + 1
        _COLS["create"] = _mr_find_col(_df_host, ["Create Date","Deal Create Date","Deal Created Date","Create_Date"])
        _COLS["pay"] = _mr_find_col(_df_host, ["Payment Received Date","Enrollment Date"])
        _COLS["calib_booking"] = _mr_find_col(_df_host, ["[Trigger] - Calibration Booking Date","Calibration Booking Date","Trigger - Calibration Booking Date"])
        _COLS["calib_done"] = _mr_find_col(_df_host, ["Calibration Done","Calibration Done Date","Calibration Completed Date"])
        _COLS["referral_intent"] = _mr_find_col(_df_host, ["Referral Intent Source"])
        _COLS["deal_source"] = _mr_find_col(_df_host, ["JetLearn Deal Source","Original Source","Original Traffic Source"])
        _COLS["ac"] = _mr_find_col(_df_host, ["Academic Counsellor","AC","Counsellor"])
        _COLS["country"] = _mr_find_col(_df_host, ["Country"])

    # Corrected exact trial columns
    _FIRST_TRIAL = _mr_find_col(_df_host, ["First Calibration Scheduled Date"]) or _mr_find_col(_df_host, ["First Trial Scheduled","First Trial Schedule Date","Trial Scheduled Date","First Calibration Scheduled"])
    _TRIAL_RESCH = _mr_find_col(_df_host, ["Calibration Rescheduled Date"]) or _mr_find_col(_df_host, ["Trial Rescheduled","Trial Re-Scheduled Date","Trial Reschedule Date","Calibration Rescheduled"])

    # Parse dates if needed
    for _c in [_COLS.get("create"), _COLS.get("pay"), _COLS.get("calib_booking"), _COLS.get("calib_done"), _FIRST_TRIAL, _TRIAL_RESCH]:
        if _c and _c in _df_host.columns and not _np_mr.issubdtype(_df_host[_c].dtype, _np_mr.datetime64):
            _df_host[_c] = _mr_to_dt(_df_host[_c])

    # Derived flags
    _df_host = _df_host.copy()
    _df_host["_self_generated"] = (_df_host[_COLS["referral_intent"]]=="Sales Generated") if _COLS.get("referral_intent") else False
    if _COLS.get("deal_source"):
    ...
    # Controls
    with _st_mr.sidebar:
        _mode = _st_mr.radio("Mode", ["MTD","Cohort"], horizontal=True, key=_MR_KEY+"mode")
        _preset = _st_mr.selectbox("Date preset", ["Today","Yesterday","This Month","Last Month","Custom"], key=_MR_KEY+"preset")
        if _preset == "Custom":
            _d1, _d2 = _st_mr.columns(2)
            with _d1:
                _start_date = _st_mr.date_input("Start date", value=_date_mr.today().replace(day=1), key=_MR_KEY+"start")
            with _d2:
                _end_date = _st_mr.date_input("End date", value=_date_mr.today(), key=_MR_KEY+"end")
        else:
            _sd, _ed = _mr_preset(_preset)
            _start_date = _sd.date(); _end_date = _ed.date()
        _gran = _st_mr.selectbox("Granularity", ["Day","Week","Month","Quarter","Year"], index=1, key=_MR_KEY+"gran")

        _st_mr.markdown("---")
        _st_mr.subheader("Splits")
        _quick = _st_mr.multiselect("Quick splits (append to group-by)",
                                    ["Academic Counsellor","JetLearn Deal Source","Country"],
                                    default=[], key=_MR_KEY+"quick")
        _st_mr.markdown("— or —")
        _dimA = _st_mr.text_input("Dimension A column name", value="", key=_MR_KEY+"dimA")
        _dimB = _st_mr.text_input("Dimension B column name", value="", key=_MR_KEY+"dimB")

        _st_mr.markdown("---")
        _st_mr.subheader("Charts")
        _chart_type = _st_mr.selectbox("Chart type", ["Bar","Stacked Bar","Line","Combo (Bar+Line)"], index=1, key=_MR_KEY+"chart_type")
        _dual = _st_mr.checkbox("Use dual Y-axes (Combo only)", key=_MR_KEY+"dual")
        _pie = _st_mr.checkbox("Show a pie chart (share by top split)", value=True, key=_MR_KEY+"pie")

    _START, _END = _pd_mr.to_datetime(_start_date), _pd_mr.to_datetime(_end_date)
    def _apply_mode_window(df_in):
        # Use the selected preset window for both modes.
        # MTD: count events whose event-dates fall within [START, END].
        # Cohort: first anchor deals by Create Date in [START, END], then count event-dates within [START, END].
        if mode == "MTD":
            return df_in, START, END
        else:
            if COLS.get("create") is None:
                return df_in.iloc[0:0], START, END
            cohort_mask = df_in[COLS["create"]].between(START, END, inclusive="both")
            return df_in.loc[cohort_mask].copy(), START, END
    ...
    # Metric registry with exact labels
    _METRICS = {
        "Deal": ("create", "Deals created"),
        "[Trigger] - Calibration Booking Date": ("calib_booking","Calibration bookings"),
        "First trial": ("first_trial","First Calibration Scheduled Date"),
        "Trial reschedule": ("trial_resched","Calibration Rescheduled Date"),
        "Total trial": ("total_trial","Union of first & reschedule (de-duped per bucket)"),
        "Calibration done": ("calib_done","Calibration done"),
        "Enrollment": ("pay","Payment Received Date"),
        "Referral": ("_referral","Deal source contains 'referr'"),
        "Self-generated": ("_self_generated","Referral Intent Source == 'Sales Generated'"),
    }

    def _metric_series(_dfs, _key, _start, _end, _freq, _idcol):
        _src = _METRICS[_key][0]
        def _ser_from_col(_col):
            if not _col or _col not in _dfs.columns: return _pd_mr.Series(dtype=float)
            _ts=_dfs[_col]; _m=_ts.notna() & (_ts>=_start) & (_ts<=_end)
            if not _m.any(): return _pd_mr.Series(dtype=float)
            _sub=_pd_mr.DataFrame({"bucket": _ts[_m].dt.to_period(_freq).dt.to_timestamp(),
                                   "id": _dfs.loc[_m, _idcol]})
            return _sub.groupby("bucket")["id"].nunique()
        if _src=="first_trial":
            return _ser_from_col(_FIRST_TRIAL)
        if _src=="trial_resched":
            return _ser_from_col(_TRIAL_RESCH)
        if _src=="total_trial":
            frames=[]
            for _c in [_FIRST_TRIAL,_TRIAL_RESCH]:
                if _c and _c in _dfs.columns:
                    _ts=_dfs[_c]; _m=_ts.notna() & (_ts>=_start) & (_ts<=_end)
                    if _m.any():
                        frames.append(_pd_mr.DataFrame({"id": _dfs.loc[_m, _idcol],
                                                        "bucket": _ts[_m].dt.to_period(_freq).dt.to_timestamp()}))
            if not frames: return _pd_mr.Series(dtype=float)
            _long=_pd_mr.concat(frames, ignore_index=True).drop_duplicates(subset=["id","bucket"])
            return _long.groupby("bucket")["id"].nunique()
        if _src=="_referral":
            if not _COLS.get("create"): return _pd_mr.Series(dtype=float)
            _mask=_dfs["_referral"]
            if not _mask.any(): return _pd_mr.Series(dtype=float)
            _ts=_dfs.loc[_mask, _COLS["create"]]; _m2=_ts.notna() & (_ts>=_start) & (_ts<=_end)
            _sub=_pd_mr.DataFrame({"bucket": _ts[_m2].dt.to_period(_freq).dt.to_timestamp(),
                                   "id": _dfs.loc[_mask].loc[_m2, _idcol]})
            if _sub.empty: return _pd_mr.Series(dtype=float)
            return _sub.groupby("bucket")["id"].nunique()
        if _src=="_self_generated":
            if not _COLS.get("create"): return _pd_mr.Series(dtype=float)
            _mask=_dfs["_self_generated"]
            if not _mask.any(): return _pd_mr.Series(dtype=float)
            _ts=_dfs.loc[_mask, _COLS["create"]]; _m2=_ts.notna() & (_ts>=_start) & (_ts<=_end)
            _sub=_pd_mr.DataFrame({"bucket": _ts[_m2].dt.to_period(_freq).dt.to_timestamp(),
                                   "id": _dfs.loc[_mask].loc[_m2, _idcol]})
            if _sub.empty: return _pd_mr.Series(dtype=float)
            return _sub.groupby("bucket")["id"].nunique()
        if _src in ("create","pay","calib_booking","calib_done"):
            return _ser_from_col(_COLS.get(_src))
        return _pd_mr.Series(dtype=float)

    def _aggregate(_dfb, _dims):
        _idc = _COLS["id"]
        _dfm, _ms, _me = _apply_mode(_dfb)
        _freq = _mr_freq(_gran)
        if len(_dims)==0:
            ...
            if _dims==["__ALL__"]:
                ...
                for _gk,_slice in _gb:
                    ...
                    if isinstance(_gk, tuple):
                        for _c,_v in zip(_dims,_gk):
                            _row[_c]=_v
                    else:
                        _row[_dims[0]]=_gk
                    _frames.append(_row)
        if not _frames:
            return _pd_mr.DataFrame(columns=["bucket"]+_dims+list(_METRICS.keys()))
        _full=_pd_mr.concat(_frames, ignore_index=True)
        _long_rows=[]
        for _k in _METRICS.keys():
            _sub=_full[["bucket"]+_dims+[_k]].dropna(subset=[_k]).rename(columns={_k:"value"}); _sub["metric"]=_k
            _long_rows.append(_sub)
        _long=_pd_mr.concat(_long_rows, ignore_index=True) if _long_rows else _pd_mr.DataFrame(columns=["bucket"]+_dims+["metric","value"])
    ...
    # Resolve groups & aggregate
    _groups=_resolve_groups(_df_host, _dimA, _dimB, _quick)
    _agg=_aggregate(_df_host, _groups)

    # KPIs
    _st_mr.markdown("### Overview (KPIs)")
    _keys=list(_METRICS.keys())
    _rowA,_rowB=_st_mr.columns(5),_st_mr.columns(5)
    _vals={k:int(_agg[k].sum()) if k in _agg.columns else 0 for k in _keys}
    for _i,_k in enumerate(_keys):
        _col=_rowA[_i] if _i<5 else _rowB[_i-5]
        _col.metric(_k, f"{_vals[_k]:,}")

    # Table
    _st_mr.markdown("### Master table")
    _st_mr.dataframe(_agg, use_container_width=True)
    _st_mr.download_button("Download CSV", data=_agg.to_csv(index=False), file_name="master_report.csv",
                           mime="text/csv", key=_MR_KEY+"dl")

    # Charts
    _st_mr.markdown("### Charts")
    if _agg.empty:
        _st_mr.info("No data for the selected configuration.")
    else:
        _avail=[k for k in _keys if k in _agg.columns]
        _default=_avail[:3]
        _pick=_st_mr.multiselect("Pick metrics to plot", _avail, default=_default, key=_MR_KEY+"metrics")
        def _base(_dfc):
            _b=_alt_mr.Chart(_dfc).encode(x=_alt_mr.X("bucket:T", title="Date bucket"))
            if len(_groups)>=1 and _groups[0] in _dfc.columns:
                _b=_b.encode(color=_alt_mr.Color(_groups[0]+":N"))
            return _b
        def _bar(_dfc,_ys,_stack=False):
            _b=_base(_dfc)
            if len(_ys)==1:
                return _b.mark_bar().encode(y=_alt_mr.Y(_ys[0]+":Q"))
            if _stack:
                _ml=_dfc.melt(id_vars=["bucket"]+_groups, value_vars=_ys, var_name="metric", value_name="value")
                return _alt_mr.Chart(_ml).mark_bar().encode(x="bucket:T", y="value:Q", color="metric:N", tooltip=list(_ml.columns))
            _layers=[_b.mark_bar().encode(y=_alt_mr.Y(y+":Q"), tooltip=list(_dfc.columns)) for y in _ys]
            return _alt_mr.layer(*_layers)
        def _line(_dfc,_ys):
            _b=_base(_dfc)
            _layers=[_b.mark_line(point=True).encode(y=_alt_mr.Y(y+":Q"), tooltip=list(_dfc.columns)) for y in _ys]
            return _alt_mr.layer(*_layers)

        if _pick:
            _ctype=_st_mr.session_state.get(_MR_KEY+"chart_type","Stacked Bar")
            _dual=_st_mr.session_state.get(_MR_KEY+"dual", False)
            if _ctype=="Bar":
                ...
                _left=_st_mr.multiselect("Left Y (bar)", _avail, default=_pick[:1], key=_MR_KEY+"left_y")
                _right=_st_mr.multiselect("Right Y (line)", _avail, default=_pick[1:2], key=_MR_KEY+"right_y")
                if _dual:
                    _left_layer=_alt_mr.layer(*[
                        _alt_mr.Chart(_agg).mark_bar().encode(
                            x="bucket:T",
                            y=_alt_mr.Y(f"{y}:Q", axis=_alt_mr.Axis(title="Left Y")),
                            color=(_groups[0]+":N") if len(_groups)>=1 and _groups[0] in _agg.columns else _alt_mr.value("steelblue"),
                            tooltip=list(_agg.columns)
                        ) for y in _left
                    ])
                    _right_layer=_alt_mr.layer(*[
                        _alt_mr.Chart(_agg).mark_line(point=True).encode(
                            x="bucket:T",
                            y=_alt_mr.Y(f"{y}:Q", axis=_alt_mr.Axis(title="Right Y")),
                            color=(_groups[0]+":N") if len(_groups)>=1 and _groups[0] in _agg.columns else _alt_mr.value("firebrick"),
                            tooltip=list(_agg.columns)
                        ) for y in _right
                    ]).resolve_scale(y='independent')
                    _chart=_alt_mr.layer(_left_layer,_right_layer)
                else:
                    _chart=_alt_mr.layer(_bar(_agg,_left,_stack=False), _line(_agg,_right))
            _st_mr.altair_chart(_chart, use_container_width=True)

        if _st_mr.session_state.get(_MR_KEY+"pie", True):
            _st_mr.markdown("#### Pie (share by top split at total level)")
            if len(_groups)==0 or (len(_groups)==1 and _groups[0]=="__ALL__"):
                _st_mr.caption("Add at least one split to see a pie.")
            else:
                _pm = _st_mr.selectbox("Metric for pie", _avail, index=0, key=_MR_KEY+"pie_metric")
                ...
                _pie = _alt_mr.Chart(_p).mark_arc().encode(theta=f"{_pm}:Q", color="label:N", tooltip=list(_p.columns))
                _st_mr.altair_chart(_pie, use_container_width=True)

    # Conversion % composer
    _st_mr.markdown("### Conversion % composer")
    if _agg.empty:
        _st_mr.info("No data to compute conversion.")
    else:
        _num = _st_mr.selectbox("Numerator metric", _keys, index=_keys.index("Enrollment"), key=_MR_KEY+"num")
        _den = _st_mr.selectbox("Denominator metric", _keys, index=_keys.index("Total trial"), key=_MR_KEY+"den")
        _dfp=_agg.copy()
        _dfp["%"] = (_dfp[_num].astype(float) / _dfp[_den].astype(float).replace(0, _np_mr.nan)) * 100.0
        _st_mr.dataframe(_dfp[["bucket"]+_groups+[_num,_den,"%"]], use_container_width=True, key=_MR_KEY+"pct_tbl")
        _pct = _alt_mr.Chart(_dfp).mark_line(point=True).encode(
            x="bucket:T",
            y=_alt_mr.Y("%:Q", title=f"{_num} / {_den} (%)"),
            color=(_groups[0]+":N") if len(_groups)>=1 and _groups[0] in _dfp.columns else _alt_mr.value("gray"),
            tooltip=list(_dfp.columns),
        )
        _st_mr.altair_chart(_pct, use_container_width=True, key=_MR_KEY+"pct_chart")
# ====================== END: Master report (INLINE, corrected columns) ======================



# --- BEGIN: Master report dispatch (robust) ---

def __mr_pick_df(_g):
    # Safely pick the first available dataframe from common names without boolean-chaining
    for _name in ("df_f","df","df_base","df_final"):
        if _name in _g:
            _cand = _g[_name]
            try:
                import pandas as __p
                if isinstance(_cand, __p.DataFrame):
                    return _cand
            except Exception:
                return _cand
    return None
...
try:
    # Style A: explicit 'view' routing
    if 'view' in globals() and isinstance(view, str) and view == "Master report":
        _df_pref = __mr_pick_df(globals())
        render_master_report(_df_pref)
    # Style B: explicit 'cur_sub' routing
    elif 'cur_sub' in globals() and isinstance(cur_sub, str) and cur_sub == "Master report":
        _df_pref = __mr_pick_df(globals())
        render_master_report(_df_pref)
    # Style C: session-state radio key routing
    else:
        for _k in ("Funnel&Movement_pill","Funnel & Movement_pill","Funnel_pill","funnel_pill","nav_sub","nav_master"):
            if _k in st.session_state:
                _val = st.session_state[_k]
                if isinstance(_val, str) and _val == "Master report":
                    _df_pref = __mr_pick_df(globals())
                    render_master_report(_df_pref)
                    break
except Exception as _e:
    st.error(f"Master report failed: {_e}")
# --- END: Master report dispatch (robust) ---
