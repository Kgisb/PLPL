...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward", "Referral Pitched In", "Closed Lost Analysis", "Trial Trend", "Booking Analysis", "Sibling Pie"],
...
try:
    ...
    if _master == "Funnel & Movement" and _view == "Sibling Pie":
        if "_render_funnel_sibling_pie" in globals():
            _render_funnel_sibling_pie(
            df_f=df_f,
            sibling_col_hint="Sibling Deal",
            stage_col_hint="Deal Stage",
            ris_col_hint="Referral Intent Source"
        )
        else:
            import streamlit as st
            st.error("Sibling Pie renderer not found in scope.")
    import streamlit as st
    _master = master if 'master' in globals() else st.session_state.get('nav_master', 'Performance')
    _view = st.session_state.get('nav_sub', '')
...
# === Funnel: Sibling Pie ===
def _render_funnel_sibling_pie(df, sibling_col_hint="Sibling Deal", stage_col_hint="Deal Stage", ris_col_hint="Referral Intent Source"):
    import pandas as _pd, streamlit as st, numpy as _np, altair as alt
    from datetime import date, timedelta
    ...
    # Column resolution
    sibling_col = sibling_col_hint if sibling_col_hint in df.columns else None
    stage_col = stage_col_hint if stage_col_hint in df.columns else None
    ris_col = ris_col_hint if ris_col_hint in df.columns else None

    # Resolve date columns
    create_candidates = ["Create Date","Created Date","Deal Create Date","CreateDate","Created On"]
    pay_candidates = ["Payment Received Date","Payment Date","Enrolment Date","PaymentReceivedDate","Paid On"]
    create_col = None
    pay_col = None
    ...
    if not sibling_col or not stage_col or not create_col:
        st.warning("Sibling Deal, Deal Stage or Create Date column missing."); return

    # --- Controls: Mode + Date preset ---
    c1, c2, c3 = st.columns([1,1,2])
    with c1:
        mode = st.radio("Mode", ["MTD","Cohort"], index=0, key="siblingpie_mode")
    with c2:
        preset = st.selectbox("Date", ["Yesterday","Today","This Month","Last Month","Custom"], index=2, key="siblingpie_preset")
    with c3:
        gran = st.selectbox("Granularity", ["Day","Week","Month"], index=0, key="siblingpie_gran")

    # Determine date basis
    date_basis = create_col
    if mode == "Cohort" and pay_col:
        date_basis = pay_col

    # Parse dates day-first
    _df = df.copy()
    _df[date_basis] = _pd.to_datetime(_df[date_basis], errors="coerce", dayfirst=True)

    today = date.today()
    if preset == "Yesterday":
        date_start = today - timedelta(days=1); date_end = date_start
    elif preset == "Today":
        date_start = today; date_end = today
    elif preset == "This Month":
        date_start = today.replace(day=1); date_end = today
    elif preset == "Last Month":
        first_this = today.replace(day=1)
        date_end = first_this - timedelta(days=1)
        date_start = date_end.replace(day=1)
    else:
        c4, c5 = st.columns(2)
        with c4: date_start = st.date_input("From", value=today.replace(day=1), key="siblingpie_from")
        with c5: date_end = st.date_input("To", value=today, key="siblingpie_to")
        if date_end < date_start:
            st.warning("End date is before start date; swapping automatically.")
            date_start, date_end = date_end, date_start

    # Apply date filter (inclusive)
    _mask = _df[date_basis].dt.date.between(date_start, date_end, inclusive="both")
    _df = _df[_mask]

    # RIS filter
    if ris_col:
        vals = sorted([v for v in _df[ris_col].dropna().unique().tolist() if str(v).strip() != ""])
        show_vals = ["All"] + vals
        selected = st.multiselect("Referral Intent Source", show_vals, default=["All"], key="siblingpie_ris")
        if "All" not in selected:
            _df = _df[_df[ris_col].isin(selected)]

    # Handle blanks
    _df[sibling_col] = _df[sibling_col].fillna("(Blank)")
    _df[stage_col] = _df[stage_col].fillna("(Unknown)")

    # Aggregation
    grouped = _df.groupby([stage_col, sibling_col]).size().reset_index(name="Count")

    # Pivot for table
    pt = grouped.pivot_table(index=stage_col, columns=sibling_col, values="Count", fill_value=0, aggfunc="sum")
    pt["Total"] = pt.sum(axis=1)
    pt.loc["Total"] = pt.sum()

    st.subheader("Sibling Pie – Deal Stage × Sibling Deal")
    st.caption(f"Date basis: **{date_basis}**, Window: **{date_start} → {date_end}**, Mode: **{mode}**")
    st.dataframe(pt)

    # KPIs row
    k1, k2, k3 = st.columns(3)
    with k1: st.metric("Total Deals in scope", len(_df))
    with k2: st.metric("Sibling rows", _df[sibling_col].notna().sum())
    with k3: st.metric("Distinct sibling values", int(_df[sibling_col].nunique()))

    # Chart
    df_long = grouped.copy()
    if not df_long.empty:
        if gran == "Day":
            x_field = f"{sibling_col}:N"
        elif gran == "Week":
            x_field = f"{sibling_col}:N"
        else:
            x_field = f"{sibling_col}:N"
        ...
        chart = alt.Chart(df_long).mark_bar().encode(
            x=alt.X(x_field, sort="-y", title="Sibling Deal"),
            y=alt.Y("Count:Q", title="Count"),
            color=alt.Color(f"{stage_col}:N", legend=alt.Legend(title="Deal Stage")),
            tooltip=[sibling_col, stage_col, "Count"]
        ).properties(height=340)
        st.altair_chart(chart, use_container_width=True)
    else:
        st.info("No counts to chart for the current window.")
    ...
    st.download_button("⬇️ Download CSV", data=csv, file_name="sibling_pie_deal_stage.csv", mime="text/csv")
