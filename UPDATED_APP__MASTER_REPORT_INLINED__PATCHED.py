...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Master report", "Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward", "Referral Pitched In", "Closed Lost Analysis", "Booking Analysis", "Trial Trend"],
...
# ====================== BEGIN: Master report (INLINE) ======================
import pandas as _pd_inline
import numpy as _np_inline
import altair as _alt_inline
import streamlit as _st_inline
from datetime import date as _date_inline, timedelta as _timedelta_inline

_JL_MR_KEY = "Funnel&Movement_master_report_"

def _jl_mr_find_col(_df, _cands):
    for _c in _cands:
        if _c in _df.columns: return _c
        for _col in _df.columns:
            if _col.strip().lower() == _c.strip().lower():
                return _col
    return None

def _jl_mr_to_dt(_s):
    return _pd_inline.to_datetime(_s, errors="coerce", dayfirst=True)

def _jl_mr_freq(_g):
    return {"Day":"D","Week":"W","Month":"M","Quarter":"Q","Year":"Y"}[_g]

def _jl_mr_preset(_preset, _today=None):
    if _today is None: _today = _date_inline.today()
    if _preset=="Today":
        _start=_today; _end=_today
    elif _preset=="Yesterday":
        _start=_today-_timedelta_inline(days=1); _end=_today-_timedelta_inline(days=1)
    elif _preset=="This Month":
        _start=_today.replace(day=1); _end=_today
    elif _preset=="Last Month":
        _first_this=_today.replace(day=1)
        _last_end=_first_this-_timedelta_inline(days=1)
        _start=_last_end.replace(day=1); _end=_last_end
    else:
        _start=_today.replace(day=1); _end=_today
    return _pd_inline.to_datetime(_start), _pd_inline.to_datetime(_end)

def render_master_report(_df_host):
    _st_inline.markdown("## Master report")
    if _df_host is None or len(_df_host)==0:
        _st_inline.info("No data available in current filter context.")
        return

    # Column map
    _COLS = {}
    _COLS["id"] = _jl_mr_find_col(_df_host, ["Record ID","Deal ID","ID","_id"])
    if _COLS["id"] is None:
        _df_host = _df_host.copy()
        _df_host["_synthetic_id"] = _np_inline.arange(len(_df_host)) + 1
        _COLS["id"] = "_synthetic_id"
    _COLS["create"] = _jl_mr_find_col(_df_host, ["Create Date","Deal Create Date","Deal Created Date","Create_Date"])
    _COLS["pay"] = _jl_mr_find_col(_df_host, ["Payment Received Date","Enrollment Date"])
    _COLS["calib_booking"] = _jl_mr_find_col(_df_host, ["[Trigger] - Calibration Booking Date","Calibration Booking Date","Trigger - Calibration Booking Date"])
    _COLS["first_cal_sched"] = _jl_mr_find_col(_df_host, ["First Calibration Scheduled","Calibration Scheduled Date","First Calibration Schedule Date"])
    _COLS["first_trial_sched"] = _jl_mr_find_col(_df_host, ["First Trial Scheduled","Trial Scheduled Date","First Trial Schedule Date"])
    _COLS["trial_resched"] = _jl_mr_find_col(_df_host, ["Trial Rescheduled","Trial Re-Scheduled Date","Trial Reschedule Date"])
    _COLS["calib_done"] = _jl_mr_find_col(_df_host, ["Calibration Done","Calibration Completed Date","Calibration Done Date"])
    _COLS["referral_intent"] = _jl_mr_find_col(_df_host, ["Referral Intent Source"])
    _COLS["deal_source"] = _jl_mr_find_col(_df_host, ["JetLearn Deal Source","Original Source","Original Traffic Source"])
    _COLS["ac"] = _jl_mr_find_col(_df_host, ["Academic Counsellor","AC","Counsellor"])
    _COLS["country"] = _jl_mr_find_col(_df_host, ["Country"])
    _COLS["deal_stage"] = _jl_mr_find_col(_df_host, ["Deal Stage","Stage"])

    # Parse dates safely
    for _k in ["create","pay","calib_booking","first_cal_sched","first_trial_sched","trial_resched","calib_done"]:
        _c = _COLS.get(_k)
        if _c and _c in _df_host.columns and not _np_inline.issubdtype(_df_host[_c].dtype, _np_inline.datetime64):
            _df_host[_c] = _jl_mr_to_dt(_df_host[_c])

    # Derived flags
    _df_host = _df_host.copy()
    _df_host["_self_generated"] = (_df_host[_COLS["referral_intent"]]=="Sales Generated") if _COLS["referral_intent"] else False
    if _COLS["deal_source"]:
        _ds = _df_host[_COLS["deal_source"]].astype(str).str.lower()
        _df_host["_referral"] = _ds.str.contains("referr")
    else:
        _df_host["_referral"] = False

    # Sidebar controls
    with _st_inline.sidebar:
        _mode = _st_inline.radio("Mode", ["MTD","Cohort"], horizontal=True, key=_JL_MR_KEY+"mode")
        _preset = _st_inline.selectbox("Date preset", ["Today","Yesterday","This Month","Last Month","Custom"], key=_JL_MR_KEY+"preset")
        if _preset == "Custom":
            _d1, _d2 = _st_inline.columns(2)
            with _d1:
                _start_date = _st_inline.date_input("Start date", value=_date_inline.today().replace(day=1), key=_JL_MR_KEY+"start")
            with _d2:
                _end_date = _st_inline.date_input("End date", value=_date_inline.today(), key=_JL_MR_KEY+"end")
        else:
            _sd, _ed = _jl_mr_preset(_preset)
            _start_date = _sd.date(); _end_date = _ed.date()
        _gran = _st_inline.selectbox("Granularity", ["Day","Week","Month","Quarter","Year"], index=1, key=_JL_MR_KEY+"gran")

        _st_inline.markdown("---")
        _st_inline.subheader("Splits")
        _quick = _st_inline.multiselect("Quick splits (append to group-by)",
                                        ["Academic Counsellor","JetLearn Deal Source","Country"],
                                        default=[], key=_JL_MR_KEY+"quick")
        _st_inline.markdown("— or —")
        _dimA = _st_inline.text_input("Dimension A column name", value="", placeholder="e.g., Academic Counsellor", key=_JL_MR_KEY+"dimA")
        _dimB = _st_inline.text_input("Dimension B column name", value="", placeholder="e.g., JetLearn Deal Source", key=_JL_MR_KEY+"dimB")

        _st_inline.markdown("---")
        _st_inline.subheader("Charts")
        _chart_type = _st_inline.selectbox("Chart type", ["Bar","Stacked Bar","Line","Combo (Bar+Line)"], index=1, key=_JL_MR_KEY+"chart_type")
        _dual = _st_inline.checkbox("Use dual Y-axes (Combo only)", key=_JL_MR_KEY+"dual")
        _pie = _st_inline.checkbox("Show a pie chart (share by top split)", value=True, key=_JL_MR_KEY+"pie")

    _START, _END = _pd_inline.to_datetime(_start_date), _pd_inline.to_datetime(_end_date)

    def _apply_mode_window(_df_in):
        if _mode == "MTD":
            _first_m = _pd_inline.Timestamp(_date_inline.today().replace(day=1))
            return _df_in, _first_m, _pd_inline.Timestamp(_date_inline.today())
        else:
            if _COLS.get("create") is None:
                return _df_in.iloc[0:0], _START, _END
            _mask = _df_in[_COLS["create"]].between(_START, _END, inclusive="both")
            return _df_in.loc[_mask].copy(), _START, _END

    _df_host["_overall_trial"] = (
        (_df_host[_COLS["first_trial_sched"]].notna() if _COLS.get("first_trial_sched") else False) |
        (_df_host[_COLS["trial_resched"]].notna() if _COLS.get("trial_resched") else False)
    )

    _METRICS = {
        "Deal": ("create", "Deals created"),
        "[Trigger] - Calibration Booking Date": ("calib_booking","Calibration bookings"),
        "First Calibration Scheduled": ("first_cal_sched","First cal scheduled"),
        "First Trial Scheduled": ("first_trial_sched","First trial scheduled"),
        "Trial Rescheduled": ("trial_resched","Trial rescheduled"),
        "Calibration Done": ("calib_done","Calibration done"),
        "Enrollment": ("pay","Enrollments (Payment Received)"),
        "Referral": ("_referral","Referral deals (flag)"),
        "Self-Generated": ("_self_generated","Self-generated (Sales Generated)"),
        "Overall Trial": ("_overall_trial","Overall trial (union)"),
    }

    def _resolve_groups(_df_in, _a, _b, _qs):
        _g = []
        if "Academic Counsellor" in _qs and _COLS.get("ac"): _g.append(_COLS["ac"])
        if "JetLearn Deal Source" in _qs and _COLS.get("deal_source"): _g.append(_COLS["deal_source"])
        if "Country" in _qs and _COLS.get("country"): _g.append(_COLS["country"])
        for _d in [_a, _b]:
            _d = (_d or "").strip()
            if _d:
                _col = _jl_mr_find_col(_df_in, [_d])
                if _col: _g.append(_col)
        _seen=set(); _final=[]
        for _x in _g:
            if _x and _x not in _seen:
                _final.append(_x); _seen.add(_x)
        return _final

    def _metric_series(_dfs, _key, _start, _end, _freq, _id):
        _src = _METRICS[_key][0]
        if _src == "_referral":
            _ts = _dfs[_COLS["create"]] if _COLS.get("create") else _pd_inline.NaT
            _m = _dfs["_referral"]; _ts=_ts[_m]; _ids=_dfs.loc[_m, _id]
        elif _src == "_self_generated":
            _ts = _dfs[_COLS["create"]] if _COLS.get("create") else _pd_inline.NaT
            _m = _dfs["_self_generated"]; _ts=_ts[_m]; _ids=_dfs.loc[_m, _id]
        elif _src == "_overall_trial":
            _idc = _COLS["id"]
            _t1 = _dfs[_COLS["first_trial_sched"]] if _COLS.get("first_trial_sched") else _pd_inline.NaT
            _t2 = _dfs[_COLS["trial_resched"]] if _COLS.get("trial_resched") else _pd_inline.NaT
            _s1 = _pd_inline.DataFrame({"id": _dfs[_idc], "dt": _t1})
            _s2 = _pd_inline.DataFrame({"id": _dfs[_idc], "dt": _t2})
            _long = _pd_inline.concat([_s1,_s2], ignore_index=True).dropna(subset=["dt"])
            _long = _long[(_long["dt"]>=_start) & (_long["dt"]<=_end)]
            _long["bucket"] = _long["dt"].dt.to_period(_freq).dt.to_timestamp()
            _long = _long.drop_duplicates(subset=["id","bucket"])
            return _long.groupby("bucket")["id"].nunique()
        else:
            _srcc = _COLS.get(_src)
            if isinstance(_srcc, str) and _srcc in _dfs.columns:
                _ts = _dfs[_srcc]; _ids=_dfs[_COLS["id"]]
            else:
                return _pd_inline.Series(dtype=float)
        _mask = _ts.notna() & (_ts>=_start) & (_ts<=_end)
        _sub = _pd_inline.DataFrame({"bucket": _ts[_mask].dt.to_period(_freq).dt.to_timestamp(), "id": _ids[_mask]})
        if _sub.empty: return _pd_inline.Series(dtype=float)
        return _sub.groupby("bucket")["id"].nunique()

    def _aggregate(_dfb, _dims):
        _idc = _COLS["id"]
        _dfm, _ms, _me = _apply_mode_window(_dfb)
        _freq = _jl_mr_freq(_gran)
        if len(_dims)==0:
            _dims=["__ALL__"]; _dfm=_dfm.copy(); _dfm["__ALL__"]="All"
        _frames=[]
        for _k in _METRICS.keys():
            if _dims==["__ALL__"]:
                _s = _metric_series(_dfm, _k, _ms, _me, _freq, _idc).rename(_k)
                _out = _s.to_frame().reset_index().rename(columns={"index":"bucket"})
                _out["__ALL__"]="All"; _frames.append(_out)
            else:
                _gb = _dfm.groupby(_dims, dropna=False, sort=False)
                for _gkeys, _slice in _gb:
                    _s = _metric_series(_slice, _k, _ms, _me, _freq, _idc).rename(_k)
                    if _s.empty: continue
                    _row = _s.to_frame().reset_index().rename(columns={"index":"bucket"})
                    if isinstance(_gkeys, tuple):
                        for _c,_v in zip(_dims, _gkeys):
                            _row[_c]=_v
                    else:
                        _row[_dims[0]]=_gkeys
                    _frames.append(_row)
        if not _frames:
            return _pd_inline.DataFrame(columns=["bucket"]+_dims+list(_METRICS.keys()))
        _full = _pd_inline.concat(_frames, ignore_index=True)
        _long_rows=[]
        for _k in _METRICS.keys():
            _sub=_full[["bucket"]+_dims+[_k]].dropna(subset=[_k]).rename(columns={_k:"value"})
            _sub["metric"]=_k; _long_rows.append(_sub)
        if not _long_rows:
            return _pd_inline.DataFrame(columns=["bucket"]+_dims+list(_METRICS.keys()))
        _long=_pd_inline.concat(_long_rows, ignore_index=True)
        _wide=_long.pivot_table(index=["bucket"]+_dims, columns="metric", values="value", aggfunc="sum").reset_index()
        for _k in _METRICS.keys():
            if _k not in _wide.columns: _wide[_k]=0
        return _wide.sort_values("bucket")

    _groups = _resolve_groups(_df_host, _dimA, _dimB, _quick)
    _agg = _aggregate(_df_host, _groups)

    # KPIs
    _st_inline.markdown("### Overview (KPIs)")
    _keys=list(_METRICS.keys())
    _rowA, _rowB = _st_inline.columns(5), _st_inline.columns(5)
    _kvals={k:int(_agg[k].sum()) if k in _agg.columns else 0 for k in _keys}
    for _i,_k in enumerate(_keys):
        _col = _rowA[_i] if _i<5 else _rowB[_i-5]
        _col.metric(_k, f"{_kvals[_k]:,}")

    # Table
    _st_inline.markdown("### Master table")
    _st_inline.dataframe(_agg, use_container_width=True)
    _st_inline.download_button("Download CSV", data=_agg.to_csv(index=False), file_name="master_report.csv",
                               mime="text/csv", key=_JL_MR_KEY+"dl")

    # Charts
    _st_inline.markdown("### Charts")
    if _agg.empty:
        _st_inline.info("No data for the selected configuration.")
    else:
        _avail=[k for k in _keys if k in _agg.columns]
        _default=_avail[:3]
        _pick=_st_inline.multiselect("Pick metrics to plot", _avail, default=_default, key=_JL_MR_KEY+"metrics")
        if _pick:
            def _base(_dfc):
                _b=_alt_inline.Chart(_dfc).encode(x=_alt_inline.X("bucket:T", title="Date bucket"))
                if len(_groups)>=1 and _groups[0] in _dfc.columns:
                    _b=_b.encode(color=_alt_inline.Color(_groups[0]+":N"))
                return _b
            def _bar(_dfc, _ys, _stack=False):
                _b=_base(_dfc)
                if len(_ys)==1:
                    return _b.mark_bar().encode(y=_alt_inline.Y(_ys[0]+":Q"))
                if _stack:
                    _ml=_dfc.melt(id_vars=["bucket"]+_groups, value_vars=_ys, var_name="metric", value_name="value")
                    return _alt_inline.Chart(_ml).mark_bar().encode(x="bucket:T", y="value:Q", color="metric:N", tooltip=list(_ml.columns))
                _layers=[_b.mark_bar().encode(y=_alt_inline.Y(y+":Q"), tooltip=list(_dfc.columns)) for y in _ys]
                return _alt_inline.layer(*_layers)
            def _line(_dfc, _ys):
                _b=_base(_dfc)
                _layers=[_b.mark_line(point=True).encode(y=_alt_inline.Y(y+":Q"), tooltip=list(_dfc.columns)) for y in _ys]
                return _alt_inline.layer(*_layers)
            _ctype=_st_inline.session_state.get(_JL_MR_KEY+"chart_type", "Stacked Bar")
            _dual=_st_inline.session_state.get(_JL_MR_KEY+"dual", False)
            if _ctype=="Bar":
                _chart=_bar(_agg, _pick, _stack=False)
            elif _ctype=="Stacked Bar":
                _chart=_bar(_agg, _pick, _stack=True)
            elif _ctype=="Line":
                _chart=_line(_agg, _pick)
            else:
                _left=_st_inline.multiselect("Left Y (bar)", _avail, default=_pick[:1], key=_JL_MR_KEY+"left_y")
                _right=_st_inline.multiselect("Right Y (line)", _avail, default=_pick[1:2], key=_JL_MR_KEY+"right_y")
                if _dual:
                    _left_layer=_alt_inline.layer(*[
                        _alt_inline.Chart(_agg).mark_bar().encode(
                            x="bucket:T",
                            y=_alt_inline.Y(f"{y}:Q", axis=_alt_inline.Axis(title="Left Y")),
                            color=(_groups[0]+":N") if len(_groups)>=1 and _groups[0] in _agg.columns else _alt_inline.value("steelblue"),
                            tooltip=list(_agg.columns)
                        ) for y in _left
                    ])
                    _right_layer=_alt_inline.layer(*[
                        _alt_inline.Chart(_agg).mark_line(point=True).encode(
                            x="bucket:T",
                            y=_alt_inline.Y(f"{y}:Q", axis=_alt_inline.Axis(title="Right Y")),
                            color=(_groups[0]+":N") if len(_groups)>=1 and _groups[0] in _agg.columns else _alt_inline.value("firebrick"),
                            tooltip=list(_agg.columns)
                        ) for y in _right
                    ]).resolve_scale(y='independent')
                    _chart=_alt_inline.layer(_left_layer, _right_layer)
                else:
                    _chart=_alt_inline.layer(_bar(_agg, _left, _stack=False), _line(_agg, _right))
            _st_inline.altair_chart(_chart, use_container_width=True)

        # Pie
        if _st_inline.session_state.get(_JL_MR_KEY+"pie", True):
            _st_inline.markdown("#### Pie (share by top split at total level)")
            if len(_groups)==0 or (len(_groups)==1 and _groups[0]=="__ALL__"):
                _st_inline.caption("Add at least one split to see a pie.")
            else:
                _pm = _st_inline.selectbox("Metric for pie", _avail, index=0, key=_JL_MR_KEY+"pie_metric")
                _p = _agg.groupby(_groups, dropna=False)[_pm].sum().reset_index()
                _p["label"] = _p[_groups].astype(str).agg(" | ".join, axis=1) if len(_groups)>1 else _p[_groups[0]].astype(str)
                _pie = _alt_inline.Chart(_p).mark_arc().encode(theta=f"{_pm}:Q", color="label:N", tooltip=list(_p.columns))
                _st_inline.altair_chart(_pie, use_container_width=True)

    # % Composer
    _st_inline.markdown("### Conversion % composer")
    if _agg.empty:
        _st_inline.info("No data to compute conversion.")
    else:
        _num = _st_inline.selectbox("Numerator metric", _keys, index=5, key=_JL_MR_KEY+"num")
        _den = _st_inline.selectbox("Denominator metric", _keys, index=9, key=_JL_MR_KEY+"den")
        _dfp=_agg.copy()
        _dfp["%"] = (_dfp[_num].astype(float) / _dfp[_den].astype(float).replace(0, _np_inline.nan)) * 100.0
        _st_inline.dataframe(_dfp[["bucket"]+_groups+[_num,_den,"%"]], use_container_width=True, key=_JL_MR_KEY+"pct_tbl")
        _pct = _alt_inline.Chart(_dfp).mark_line(point=True).encode(
            x="bucket:T",
            y=_alt_inline.Y("%:Q", title=f"{_num} / {_den} (%)"),
            color=(_groups[0]+":N") if len(_groups)>=1 and _groups[0] in _dfp.columns else _alt_inline.value("gray"),
            tooltip=list(_dfp.columns),
        )
        _st_inline.altair_chart(_pct, use_container_width=True, key=_JL_MR_KEY+"pct_chart")
# ====================== END: Master report (INLINE) ======================



# --- BEGIN: Master report dispatch (INLINE) ---
try:
    # Try common session-state keys used to store selected pill under Funnel & Movement
    _mr_candidates = []
    for _k in ("Funnel&Movement_pill","Funnel & Movement_pill","Funnel_pill","funnel_pill","pill"):
        if _k in st.session_state:
            _mr_candidates.append(st.session_state[_k])
    _mr_selected = next((v for v in _mr_candidates if isinstance(v, str)), None)
    if _mr_selected == "Master report":
        _mr_df = None
        for _cand in ("df_f","df","df_base","df_final"):
            if _cand in globals():
                _mr_df = globals()[_cand]; break
        render_master_report(_mr_df if _mr_df is not None else df)
except Exception as _e:
    st.error(f"Master report failed to render: {_e}")
# --- END: Master report dispatch (INLINE) ---



# === Master report view handler (added) ===
try:
    if view == "Master report":
        _df_pref = globals().get('df_f', None)
        render_master_report(_df_pref if _df_pref is not None else globals().get('df', None))
except Exception as e:
    st.error(f"Master report failed: {e}")
# === End handler ===
