...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward", "Referral Pitched In", "Closed Lost Analysis", "Trial Trend"],
...
def _render_marketing_sibling_ds(
    ...
    sibling_col = _resolve(sibling_col_hint, ["Sibling Deal","Sibling D","Sibling","Sibling_DS","sibling","sibling deal","sibling_d"])
    create_col = _resolve(create_col_hint, ["Create Date","Create_Date","Created At","Deal Create Date","create date","create_date"])
    payment_col = _resolve(payment_col_hint, ["Payment Received Date","Enrollment Date","Payment Date","payment received date","payment_date"])
    first_trial_col = _resolve(first_trial_col_hint, ["First Trial Scheduled Date","First Calibration Scheduled Date","Trial Scheduled Date","trial scheduled date"])
    resched_col = _resolve(resched_col_hint, ["Trial Rescheduled Date","Calibration Rescheduled Date","trial rescheduled date"])
    trial_done_col = _resolve(trial_done_col_hint, ["Trial Done Date","First Trial Done Date","trial done date"])
    cal_done_col = _resolve(cal_done_col_hint, ["Calibration Done Date","Trial Done Date (Calibration)","calibration done date"])
    ris_col = _resolve(referral_intent_col_hint, ["Referral Intent Source","Referral Intent","referral intent source","RIS"])

    if sibling_col is None:
        st.error("Sibling column not found. Expected one of: Sibling Deal / Sibling D / Sibling / Sibling_DS")
        return
    ...
    # ---- Controls ----
    cols = st.columns([1.2,1.2,1,1.4,1])
    with cols[0]:
    ...
    # end end_d inclusive adjustment:
    ...
    def _event_col_for_metric(metric_name):
        ...
        if metric_name == "Sales Generated (RIS)":
            # tie to Create to count deals created that are RIS
            return create_col
        # Biggest Stage is not tied to a single date; use Create as cohort anchor
        return create_col
    ...
    # drop NA event rows when metric needs an event column
    needs_event = metric not in ["Biggest Stage","Total"]  # for Total, use create-based window
    ...
    if ev_col is None:
        ...
        base = base.iloc[0:0]
    if mode == "MTD":
        # both event date within window AND create date within window
        ...
        # Cohort: only event date in window (or Create for Total/Biggest Stage)
    ...
    if metric == "Biggest Stage":
        stage_col = _resolve(None, ["Deal Stage","Deal stage","Stage","deal stage"])
        if stage_col is None:
            st.error("Deal Stage column not found for 'Biggest Stage'.")
            return
        agg = (base.groupby([sibling_col, stage_col], dropna=False)
                    .size().reset_index(name="Count"))
        # tag biggest per sibling
        top = agg.sort_values(["Sibling", "Count"] if "Sibling" in agg.columns else [sibling_col, "Count"],
                              ascending=[True, False]).copy()
        # Compute top stage per sibling
    ...
    if ev_col is not None:
        ...
        # granularity
        if gran == "Day":
            grp = base.groupby([sibling_col, base[ev_col].dt.date]).size().reset_index(name="Count")
            grp.rename(columns={ev_col:"Date"}, inplace=True)
            grp["Date"] = pd.to_datetime(grp[ev_col if ev_col in grp.columns else "Date"], errors="coerce")
        elif gran == "Week":
            grp = base.copy()
            grp["Week"] = base[ev_col].dt.to_period("W").apply(lambda p: p.start_time.date())
            grp = grp.groupby([sibling_col, "Week"]).size().reset_index(name="Count")
            grp.rename(columns={"Week":"Date"}, inplace=True)
            grp["Date"] = pd.to_datetime(grp["Date"], errors="coerce")
        else:
            grp = base.copy()
            grp["Month"] = base[ev_col].dt.to_period("M").apply(lambda p: p.start_time.date())
            grp = grp.groupby([sibling_col, "Month"]).size().reset_index(name="Count")
...
# --- Patch: Register "Sibling DS" under Marketing if possible ---
try:
    if "MASTER_SECTIONS" in globals() and isinstance(MASTER_SECTIONS, dict):
        if "Marketing" in MASTER_SECTIONS and "Sibling DS" not in MASTER_SECTIONS["Marketing"]:
            MASTER_SECTIONS["Marketing"] = MASTER_SECTIONS["Marketing"] + ["Sibling DS"]
except Exception:
    pass
