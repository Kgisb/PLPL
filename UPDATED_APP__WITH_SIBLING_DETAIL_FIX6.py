...
def exclude_invalid_deals(df: pd.DataFrame, dealstage_col: Optional[str]) -> tuple[pd.DataFrame, int]:
...
def assign_src_pick(df: pd.DataFrame, source_col: Optional[str], use_key: bool) -> pd.DataFrame:
    d = df.copy()
...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Funnel & Movement": ["Funnel", "Lead Movement", "Stuck deals", "Deal Velocity", "Deal Decay", "Carry Forward", "Referral Pitched In", "Closed Lost Analysis", "Booking Analysis", "Sibling Detail", "Trial Trend"],
...
def _render_performance_sales_activity(
    df_f: pd.DataFrame,
    first_cal_sched_col: Optional[str],
    cal_resched_col: Optional[str],
    slot_col: Optional[str],
    counsellor_col: Optional[str],
...
def _render_funnel_referral_pitched_in(
    df_f,
    counsellor_col: Optional[str],
    create_col: Optional[str],
    cal_done_col: Optional[str],
    ref_pitched_col: Optional[str],
):
    import streamlit as st
    import pandas as pd
    from datetime import date, timedelta
...
elif view == "Sibling Detail":
    _render_funnel_sibling_detail(
        df_f=df_f,
        sibling_col=find_col(df, ["Sibling Deal","Sibling","Sibling_Deal","sibling deal","Sibling deal"]),
        deal_stage_col=find_col(df, ["Deal Stage","Stage","DealStage"]),
        counsellor_col=counsellor_col,
...
def _render_performance_original_source(
    df_f,
    create_col: Optional[str],
    enrol_col: Optional[str],
    drill1_col: Optional[str],
    drill2_col: Optional[str],
):
    import streamlit as st
    import pandas as pd
    import altair as alt
...
def _render_performance_quick_view(
    ...
    *,
    ...
    def _section_tables(df_source: pd.DataFrame, title: str, group_key: Optional[str], label_suffix: str, display_name: Optional[str] = None):
        with st.container():
            st.markdown(title)
            c1, c2, c3 = st.columns(3)
...
def _render_performance_lead_mix(
    df_f,
    create_col: Optional[str] = None,
    pay_col: Optional[str] = None,
    first_cal_sched_col: Optional[str] = None,
    cal_resched_col: Optional[str] = None,
    cal_done_col: Optional[str] = None,
    ...
    def _mask(df_in: pd.DataFrame, col: Optional[str]):
...
def _render_performance_referral_performance(
    ...
    create_col: Optional[str] = None,
    pay_col: Optional[str] = None,
    ref_intent_col: Optional[str] = None,
    country_col: Optional[str] = None,
    counsellor_col: Optional[str] = None,
):
    import streamlit as st
    import pandas as pd
    import numpy as np
...
def _render_performance_slow_working_deals(
    df_f,
    create_col: Optional[str] = None,
    last_activity_col: Optional[str] = None,
    country_col: Optional[str] = None,
    counsellor_col: Optional[str] = None,
    source_col: Optional[str] = None,
    times_contacted_col: Optional[str] = None,
    sales_activity_col: Optional[str] = None,
):
    import streamlit as st
    import pandas as pd
    import numpy as np
...
def _render_funnel_trial_trend(
    df_f,
    first_trial_col_hint: Optional[str] = None,
    resched_col_hint: Optional[str] = None,
    trial_done_col_hint: Optional[str] = None,
    enrol_col_hint: Optional[str] = None,
    create_col_hint: Optional[str] = None,
    counsellor_col_hint: Optional[str] = None,
    country_col_hint: Optional[str] = None,
    source_col_hint: Optional[str] = None,
):
    import streamlit as st
...
# === Funnel & Movement: Sibling Detail Renderer ===
def _render_funnel_sibling_detail(
    df_f,
    sibling_col,
    deal_stage_col,
    counsellor_col,
    ...
    st.subheader("Funnel & Movement — Sibling Detail")

    # Column checks
    if not sibling_col or sibling_col not in df_f.columns:
        st.warning("Sibling Deal column not found.", icon="⚠️"); return
    if not deal_stage_col or deal_stage_col not in df_f.columns:
        st.warning("Deal Stage column not found.", icon="⚠️"); return
    if not counsellor_col or counsellor_col not in df_f.columns:
        st.warning("Academic Counsellor column not found.", icon="⚠️"); return
    if not country_col or country_col not in df_f.columns:
        st.warning("Country column not found.", icon="⚠️"); return
    if not create_col or create_col not in df_f.columns:
        st.warning("Create Date column not found.", icon="⚠️"); return
    ...
    c1, c2 = st.columns(2)
    with c1:
        mode = st.radio("Counting mode", ["MTD","Cohort"], index=0, horizontal=True, key="sib_mode")
    with c2:
        preset = st.radio("Date scope (Create Date)", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, horizontal=True, key="sib_scope")
    ...
    if preset == "Today":
        ...
        cc1, cc2 = st.columns(2)
        with cc1:
            start_d = st.date_input("Start date", today.replace(day=1), key="sib_start")
        with cc2:
            end_d = st.date_input("End date", today, key="sib_end")

    dim_choice = st.selectbox("Breakdown by", ["Deal Stage","Academic Counsellor","Country"], index=0, key="sib_dim")
    ...
    df["_create_dt"] = _to_dt(df[create_col])

    # Filter window (Create Date based)
    mask = (df["_create_dt"] >= pd.to_datetime(start_d)) & (df["_create_dt"] <= pd.to_datetime(end_d))
    df = df.loc[mask].copy()

    # Exclude invalid stage
    if df[deal_stage_col].dtype == object:
        df = df[df[deal_stage_col].fillna("").str.strip().ne("1.2 Invalid deal(s)")]

    # Normalize sibling values
    df[sibling_col] = df[sibling_col].astype("string").fillna("(Blank)").replace({"": "(Blank)"})

    # Pick dimension
    if dim_choice == "Deal Stage":
        dim_col = deal_stage_col
    elif dim_choice == "Academic Counsellor":
        dim_col = counsellor_col
    else:
        dim_col = country_col

    # KPIs
    k1, k2, k3 = st.columns(3)
    with k1: st.metric("Deals in window", int(len(df)))
    with k2: st.metric("Distinct " + dim_choice, int(df[dim_col].nunique()))
    with k3: st.metric("Sibling value buckets", int(df[sibling_col].nunique()))

    if df.empty:
        st.info("No data for the selected period.", icon="ℹ️")
        return

    # Aggregation
    agg = (
        df.groupby([dim_col, sibling_col], dropna=False)
          .size()
          .reset_index(name="Count")
    )
    totals = agg.groupby(dim_col)["Count"].sum().rename("RowTotal")
    agg = agg.merge(totals, on=dim_col, how="left")
    agg["Row %"] = np.where(agg["RowTotal"] > 0, (agg["Count"] / agg["RowTotal"]) * 100.0, 0.0)

    st.markdown(f"**Stacked bar — {dim_choice} × Sibling Deal**")
    chart = alt.Chart(agg).mark_bar().encode(
        x=alt.X(f"{dim_col}:N", sort='-y', title=dim_choice),
        y=alt.Y("Count:Q", stack="zero", title="Count"),
        color=alt.Color(f"{sibling_col}:N", title="Sibling Deal"),
        tooltip=[dim_col, sibling_col, "Count", alt.Tooltip("Row %:Q", format=".1f")]
    ).properties(height=380)
    st.altair_chart(chart, use_container_width=True)

    table = agg.pivot_table(index=dim_col, columns=sibling_col, values="Count", aggfunc="sum", fill_value=0)
    ...
    st.download_button("Download CSV", csv, file_name=f"sibling_detail_{start_d}_to_{end_d}.csv", mime="text/csv", key="sib_dl")
