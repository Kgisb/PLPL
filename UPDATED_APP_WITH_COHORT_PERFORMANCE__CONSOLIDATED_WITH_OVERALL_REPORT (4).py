...
with st.sidebar:
    ...
    try:
        ...
        passdef _render_marketing_overall_report(df_f: pd.DataFrame):
    import pandas as pd
    import numpy as np
    import streamlit as st
    from datetime import date, timedelta
    ...
    # --- Column resolution ---
    def _col(df, names):
        return _col_exists(df, names) if ' _col_exists' or '_col_exists' in globals() else find_col(df, names)
    
    create_col   = _col(df_f, ["Create Date","Created Date","Created On","Creation Date","Deal Created Date","Create_Date"])
    pay_col      = _col(df_f, ["Payment Received Date","Enrollment Date","Enrolled On","Payment Date","Payment_Received_Date"])
    first_col    = _col(df_f, ["First Calibration Scheduled Date","First Cal Scheduled","First Calibration Scheduled"])
    resched_col  = _col(df_f, ["Calibration Rescheduled Date","Cal Rescheduled Date","Trial Rescheduled Date"])
    booking_col  = _col(df_f, ["[Trigger] - Calibration Booking Date","Calibration Booking Date","Booking Date"])
    done_col     = _col(df_f, ["Calibration Done Date","Trial Done Date"])
    ref_int_col  = _col(df_f, ["Referral Intent Source","Referral Intent","Referral Source Intent"])
    csl_col      = _col(df_f, ["Student/Academic Counsellor","Student/Academic Counselor","Academic Counselor","Counsellor","Counselor"])
    src_col      = _col(df_f, ["JetLearn Deal Source","Deal Source","Source","Original source"])
    country_col  = _col(df_f, ["Country"])
    
    # Guard
    _need = [create_col, pay_col, first_col, resched_col, booking_col, done_col, ref_int_col]
    if any(x is None for x in _need):
        st.warning("Some required columns were not found; the report will use what’s available.")
    
    # --- Global controls ---
    lcol, rcol = st.columns([1,1])
    with lcol:
        mode = st.radio("Mode", ["MTD","Cohort"], index=0, horizontal=True, key="ovr_mode")
    with rcol:
        preset = st.selectbox("Date preset", ["Today","Yesterday","This Month","Last Month","Custom"], index=2, key="ovr_preset")
    
    if preset == "Custom":
        d1, d2 = st.date_input("Custom window", value=(date.today().replace(day=1), date.today()), key="ovr_range")
    else:
        today = date.today()
        if preset == "Today":
            d1 = d2 = today
            ...
            d1 = date(last_prev.year, last_prev.month, 1)
            d2 = last_prev
        else:
            d1 = date(today.year, today.month, 1); d2 = today
    
    gran = st.selectbox("Granularity", ["Day","Week","Month","Year"], index=1, key="ovr_gran")
    breakdown = st.selectbox("Breakdown by", ["Academic Counselor","JetLearn Deal Source","Country"], index=0, key="ovr_break")
    
    # Map breakdown to column
    dim_map = {
        "Academic Counselor": csl_col or "Academic Counselor",
        "JetLearn Deal Source": src_col or "JetLearn Deal Source",
        "Country": country_col or "Country",
    }
    dim_col = dim_map[breakdown]
    
    # --- Date coercion ---
    def _dt(s):
        return coerce_datetime(df_f[s]) if s and s in df_f.columns else pd.Series(pd.NaT, index=df_f.index)
    
    d_create  = _dt(create_col)
    d_pay     = _dt(pay_col)
    d_first   = _dt(first_col)
    d_res     = _dt(resched_col)
    d_book    = _dt(booking_col)
    d_done    = _dt(done_col)
    
    # Exclude invalid deals if helper exists
    if 'INVALID_RE' in globals():
        mask_valid = ~df_f.get("Deal Stage","").astype(str).str.match(INVALID_RE, na=False)
    else:
        mask_valid = pd.Series(True, index=df_f.index)
    
    # Build base cohort
    in_create_win = (d_create.dt.date >= d1) & (d_create.dt.date <= d2)
    if mode == "MTD":
        base = df_f.loc[mask_valid & in_create_win].copy()
    else:  # Cohort
        base = df_f.loc[mask_valid & in_create_win].copy()
    
    # Helper to count events in window
    def _count_in_window(dates):
        return ((dates.dt.date >= d1) & (dates.dt.date <= d2))
    
    # Trials: union of first+resched per deal per calendar day
    deal_id = df_f.index  # fallback if no explicit ID; row index uniqueness assumed
    trial_dates = []
    # Combine into frame for melting
    tmp = pd.DataFrame({"first": d_first.dt.date, "res": d_res.dt.date}, index=df_f.index)
    tmp = tmp.loc[base.index]  # cohort restriction
    # For each row, collect unique dates among first/res if in window
    tmask_first = _count_in_window(d_first)
    tmask_res   = _count_in_window(d_res)
    trial_count = ((tmask_first | False) & base.index.isin(df_f.index)).loc[base.index].astype(int)  # init
    # Better: count unique non-null among first/res that fall in window
    trial_count = ( ((tmask_first.loc[base.index]) & d_first.loc[base.index].notna()).astype(int)
                  + ((tmask_res.loc[base.index])   & d_res.loc[base.index].notna()).astype(int) )
    # If both fall on the same calendar day, subtract 1
    same_day = (d_first.loc[base.index].dt.date == d_res.loc[base.index].dt.date) & (tmask_first.loc[base.index]) & (tmask_res.loc[base.index])
    trial_count = trial_count - same_day.astype(int)
    trial_total = int(trial_count.clip(lower=0).sum())
    
    # Event counts in window (cohort already applied via base.index)
    book_total = int(_count_in_window(d_book).loc[base.index].sum())
    done_total = int(_count_in_window(d_done).loc[base.index].sum())
    deal_total = int(base.shape[0])
    enroll_total = int(_count_in_window(d_pay).loc[base.index].sum())
    
    # Referral metrics
    ref_series = df_f[ref_int_col] if ref_int_col and ref_int_col in df_f.columns else pd.Series("", index=df_f.index)
    ref_in_base = ref_series.loc[base.index].fillna("").astype(str)
    referral_generated = int((ref_in_base != "").sum())
    sales_gen_mask = ref_in_base.str.strip().str.casefold() == "sales generated"
    sales_generated = int(sales_gen_mask.sum())
    
    # Conversions
    # For conversions, restrict numerator to enrollments within window AND ref category
    enroll_mask = _count_in_window(d_pay).loc[base.index]
    referral_enroll = int((ref_in_base != "").loc[base.index][enroll_mask].sum())
    salesgen_enroll = int((sales_gen_mask.loc[base.index] & enroll_mask).sum())
    
    referral_conv = (referral_enroll / referral_generated * 100.0) if referral_generated else 0.0
    salesgen_conv = (salesgen_enroll / sales_generated * 100.0) if sales_generated else 0.0
    l2e = (enroll_total / deal_total * 100.0) if deal_total else 0.0
    
    # --- PART A: KPI cards ---
    kpi = st.columns(5)
    kpi2 = st.columns(5)
    with kpi[0]: st.metric("Deals", f"{deal_total:,}")
    with kpi[1]: st.metric("Enrollments", f"{enroll_total:,}")
    with kpi[2]: st.metric("Total Trials", f"{trial_total:,}")
    with kpi[3]: st.metric("Bookings", f"{book_total:,}")
    with kpi[4]: st.metric("Cal. Done", f"{done_total:,}")
    with kpi2[0]: st.metric("Referral Generated", f"{referral_generated:,}")
    with kpi2[1]: st.metric("Sales Generated", f"{sales_generated:,}")
    with kpi2[2]: st.metric("Referral Conversion %", f"{referral_conv:.1f}%")
    with kpi2[3]: st.metric("Sales-Generated Conversion %", f"{salesgen_conv:.1f}%")
    with kpi2[4]: st.metric("L2E %", f"{l2e:.1f}%")
    
    st.divider()
    
    # Prepare dimension column and time bucket
    dim = df_f[dim_col].loc[base.index].fillna("Unknown").astype(str) if dim_col in df_f.columns else pd.Series("Unknown", index=base.index)
    # Build time bucket
    d_bucket = d_create.loc[base.index].copy()
    if gran == "Day":
        bucket = d_bucket.dt.to_period("D").dt.to_timestamp()
    elif gran == "Week":
        bucket = d_bucket.dt.to_period("W").dt.start_time
    elif gran == "Month":
        bucket = d_bucket.dt.to_period("M").dt.to_timestamp()
    else:
        bucket = d_bucket.dt.to_period("Y").dt.to_timestamp()
    
    # --- PART B: Explorer (counts and % selectable) ---
    st.subheader("Explorer — Data & Graphs")
    metrics = {
        "Deals":"deals",
        "Enrollments":"enroll",
        "Total Trials":"trials",
        "Bookings":"book",
        "Cal. Done":"done",
        "Referral Generated":"ref_gen",
        "Sales Generated":"sales_gen",
        "Referral Conversion %":"ref_conv",
        "Sales-Generated Conversion %":"sales_conv",
        "L2E %":"l2e"
    }
    f1 = st.multiselect("Filter-1 metrics", list(metrics.keys()), default=["Deals","Enrollments"], key="ovr_f1")
    f2 = st.multiselect("Filter-2 metrics (optional)", list(metrics.keys()), default=[], key="ovr_f2")
    
    # Build a tidy dataframe with all base signals per row
    base_df = pd.DataFrame({
        "dim": dim.values,
        "bucket": bucket.values,
        "deals": 1
    }, index=base.index)
    base_df["enroll"] = _count_in_window(d_pay).loc[base.index].astype(int).values
    base_df["book"] = _count_in_window(d_book).loc[base.index].astype(int).values
    base_df["done"] = _count_in_window(d_done).loc[base.index].astype(int).values
    # trials per row already computed as counts 0/1/2 then adjusted for same-day -> may be 0/1/2-1
    base_df["trials"] = trial_count.clip(lower=0).values
    base_df["ref_gen"] = (ref_in_base != "").astype(int).values
    base_df["sales_gen"] = sales_gen_mask.loc[base.index].astype(int).values
    
    # Aggregate
    agg = base_df.groupby(["dim","bucket"], dropna=False).sum(numeric_only=True).reset_index()
    # Derived percentages
    agg["ref_conv"] = np.where(agg["ref_gen"]>0, agg["enroll"]/agg["ref_gen"]*100.0, 0.0)
    agg["sales_conv"] = np.where(agg["sales_gen"]>0, agg["enroll"]/agg["sales_gen"]*100.0, 0.0)
    agg["l2e"] = np.where(agg["deals"]>0, agg["enroll"]/agg["deals"]*100.0, 0.0)
    
    # Show table
    cols_show = ["dim","bucket"] + [metrics[m] for m in (f1+f2) if m in metrics]
    if not cols_show[2:]:
        cols_show = ["dim","bucket","deals","enroll"]
    st.dataframe(agg[cols_show].sort_values(["dim","bucket"]), use_container_width=True)
    
    # Chart (simple line for trends; bars when a single bucket)
    try:
        import altair as alt
        melt = agg.melt(id_vars=["dim","bucket"], value_vars=cols_show[2:], var_name="metric", value_name="value")
        chart = alt.Chart(melt).mark_line(point=True).encode(
            x="bucket:T",
            y="value:Q",
            color="metric:N",
            facet=alt.Facet("dim:N", columns=1)
    ...
    # --- PART C: Derived comparative ---
    st.subheader("Derived & Comparative (%)")
    base_a = st.selectbox("Base A (denominator)", ["Deals","Referral Generated","Sales Generated"], index=0, key="ovr_cmp_a")
    base_b = st.selectbox("Base B (numerator)", ["Enrollments"], index=0, key="ovr_cmp_b")
    
    a = metrics[base_a]; b = metrics[base_b]
    cmp = agg.copy()
    cmp["pct"] = np.where(cmp[a]>0, cmp[b]/cmp[a]*100.0, 0.0)
    st.dataframe(cmp[["dim","bucket",a,b,"pct"]].sort_values(["dim","bucket"]), use_container_width=True)
    try:
        import altair as alt
        c = alt.Chart(cmp).mark_line(point=True).encode(
            x="bucket:T", y="pct:Q", color="dim:N"
        ).properties(height=220)
...
# =========================
    ...
    except Exception as _e_rpi:
        import streamlit as st
        st.error(f"Referral Pitched In failed: {type(_e_rpi).__name__}: {_e_rpi}")



elif view == "Overall Report":
    try:
        _render_marketing_overall_report(df_f=df_f)
    except Exception as _e_ovr:
        import streamlit as st
        st.error(f"Overall Report failed: {type(_e_ovr).__name__}: {_e_ovr}")
