...
from master_report_addon import render_master_report  # added: Master report pill
...
# --- BEGIN: Master report pill dispatch (safe-append) ---
try:
    _pill_value_candidates = []
    for k in ("Funnel&Movement_pill", "Funnel & Movement_pill", "Funnel_pill", "funnel_pill", "pill"):
        if k in st.session_state:
            _pill_value_candidates.append(st.session_state[k])
    _selected_pill = next((v for v in _pill_value_candidates if isinstance(v, str)), None)
    if _selected_pill == "Master report":
        try:
            # prefer a prepared/filtered dataframe if your app uses one in Funnel & Movement
            _df_pref = None
            for cand in ("df_f", "df", "df_base", "df_final"):
                if cand in globals():
                    _df_pref = globals()[cand]
                    break
            render_master_report(_df_pref if _df_pref is not None else df)
        except Exception as e:
            st.error(f"Master report failed: {e}")
except Exception as _e:
    pass
# --- END: Master report pill dispatch (safe-append) ---
