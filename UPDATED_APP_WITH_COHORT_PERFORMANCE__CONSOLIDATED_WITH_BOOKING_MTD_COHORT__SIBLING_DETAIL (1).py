...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "Sibling Detail"],
    }
...
# ==================== NEW PILL: Marketing -> Sibling Detail ====================

def _render_marketing_sibling_detail(df_f: pd.DataFrame):
    import streamlit as st
    import pandas as pd
    import numpy as np

    st.subheader("Sibling Detail")
    ...
    def _find_col(df, candidates, default=None):
        ...
        for k,v in cols.items():
            for cand in candidates:
                if cand.lower().strip() in k:
                    return v
        return default

    sibling_col = _find_col(df_f, [
        "Sibling Deal","Sibling_Deal","Sibling deal","Sibling"
    ])
    referral_col = _find_col(df_f, [
        "Referral Intent Source","Referral_Intent_Source","Referral Intent"
    ])
    pay_col = _find_col(df_f, [
        "Payment Received Date","Payment_Received_Date","Payment Date"
    ])
    create_col = _find_col(df_f, [
        "Create Date","Create_Date","Deal Create Date","Deal Created Date"
    ])
    ac_col = _find_col(df_f, [
        "Academic Counsellor","Academic_Counsellor","AC","Counsellor"
    ])
    country_col = _find_col(df_f, ["Country"])

    cal_done_col = _find_col(df_f, [
        "Calibration Done Date","Calibration_Done_Date","Calibration Done"
    ])
    cal_first_sched_col = _find_col(df_f, [
        "First Calibration Scheduled","Calibration Booking Date",
        "Calibration Booked Date","First_Calibration_Scheduled"
    ])
    cal_resched_col = _find_col(df_f, [
        "Calibration Rescheduled Date","Calibration Rescheduled",
        "Calibration_Rescheduled_Date"
    ])

    trial_sched_col = _find_col(df_f, [
        "Trial Scheduled","First Trial Scheduled","Trial_Scheduled"
    ])
    trial_resched_col = _find_col(df_f, [
        "Trial Rescheduled","Trial_Rescheduled"
    ])
    trial_done_col = _find_col(df_f, [
        "Trial Done","Trial_Done"
    ])

    # guard
    if sibling_col is None or referral_col is None:
        st.info("Required columns not found. Need both 'Sibling Deal' and 'Referral Intent Source'.")
        st.write("Detected columns:", list(df_f.columns))
        return

    # ---------- Coerce dates ----------
    def _to_ts(s):
        if s is None or s not in df_f.columns:
            return None
        out = pd.to_datetime(df_f[s], errors="coerce", dayfirst=True)
        return out

    df = df_f.copy()
    df["_create_dt"] = _to_ts(create_col)
    df["_pay_dt"] = _to_ts(pay_col)
    df["_cal_done_dt"] = _to_ts(cal_done_col)
    df["_cal_first_dt"] = _to_ts(cal_first_sched_col)
    df["_cal_resched_dt"] = _to_ts(cal_resched_col)
    df["_trial_sched_dt"] = _to_ts(trial_sched_col)
    df["_trial_resched_dt"] = _to_ts(trial_resched_col)
    df["_trial_done_dt"] = _to_ts(trial_done_col)

    # ---------- Controls ----------
    mode = st.radio("Mode", ["MTD", "Cohort"], horizontal=True, key="sd_mode")
    preset = st.selectbox(
        "Date Range",
        ["Today","Yesterday","This Month","Last Month","Custom"],
        index=2, key="sd_preset"
    )
    ...
    if preset == "Today":
        ...
        with col1:
            start = st.date_input("Start date", (today - pd.Timedelta(days=30)).date(), key="sd_start")
        with col2:
            end = st.date_input("End date", today.date(), key="sd_end")
        start = pd.to_datetime(start)
        # Make end inclusive to end-of-day
        end = pd.to_datetime(end) + pd.Timedelta(hours=23, minutes=59, seconds=59)

    gran = st.radio("Granularity", ["Day","Week","Month"], horizontal=True, key="sd_gran")
    split_orientation = st.radio("Split layout", ["Sibling Deal stacked by Referral", "Referral stacked by Sibling Deal"], horizontal=False, key="sd_split")
    view_scope = st.multiselect("Breakdown by", ["AC-wise","Country-wise"], default=[], key="sd_scope")
    ...
    if mode == "MTD":
        # Deals created in window (if create exists), and events also in window
        mask_deals = _between(df["_create_dt"]) if df["_create_dt"].notna().any() else pd.Series(True, index=df.index)
        mask_enroll = _between(df["_pay_dt"]) if df["_pay_dt"].notna().any() else pd.Series(False, index=df.index)
    else:
        # Cohort: payment inside window, irrespective of create
        mask_deals = pd.Series(True, index=df.index)  # let total deals reflect those linked to payment window when counting enrollments
        mask_enroll = _between(df["_pay_dt"]) if df["_pay_dt"].notna().any() else pd.Series(False, index=df.index)

    # Calibration masks (dedupe same-day duplicates per deal)
    def _dedup_count(ts_col):
        if ts_col is None:
            return pd.Series(False, index=df.index)
        # true where timestamp in window
        m = _between(ts_col)
        return m.fillna(False)

    m_cal_done = _dedup_count(df["_cal_done_dt"])
    m_cal_first = _dedup_count(df["_cal_first_dt"])
    m_cal_res = _dedup_count(df["_cal_resched_dt"])

    # If first and reschedule are same date for same row, count once:
    # Since one row = one deal, check equality of dates (ignoring NaT)
    same_day = (df["_cal_first_dt"].dt.normalize() == df["_cal_resched_dt"].dt.normalize())
    same_day = same_day.fillna(False)
    # unified scheduled mask inside window: either first or resched in window; if both same day, treat as one
    m_cal_scheduled = (m_cal_first | m_cal_res)
    # For rows with both in window and same day, keep as one by not double-counting later (we'll aggregate with boolean -> sum so OR already dedupes)

    # Trial masks (optional)
    m_trial_sched = _dedup_count(df["_trial_sched_dt"]) if "_trial_sched_dt" in df else pd.Series(False, index=df.index)
    m_trial_res = _dedup_count(df["_trial_resched_dt"]) if "_trial_resched_dt" in df else pd.Series(False, index=df.index)
    m_trial_done = _dedup_count(df["_trial_done_dt"]) if "_trial_done_dt" in df else pd.Series(False, index=df.index)

    # ---------- KPI summary ----------
    df_scope = df.copy()
    df_scope["__is_deal"] = mask_deals
    df_scope["__is_enroll"] = mask_enroll
    df_scope["__cal_done"] = m_cal_done
    df_scope["__cal_sched"] = m_cal_scheduled
    df_scope["__trial_sched"] = m_trial_sched
    df_scope["__trial_res"] = m_trial_res
    df_scope["__trial_done"] = m_trial_done

    total_deals = int(df_scope["__is_deal"].sum())
    total_enroll = int(df_scope["__is_enroll"].sum())
    total_cal_done = int(df_scope["__cal_done"].sum())
    total_cal_sched = int(df_scope["__cal_sched"].sum())
    total_trial = int((df_scope["__trial_sched"] | df_scope["__trial_res"]).sum()) if (m_trial_sched.any() or m_trial_res.any()) else 0
    k1,k2,k3,k4,k5 = st.columns(5)
    k1.metric("Deals", total_deals)
    k2.metric("Enrollments", total_enroll)
    k3.metric("Calibration Done", total_cal_done)
    k4.metric("Calibration Scheduled (dedup)", total_cal_sched)
    k5.metric("Trials (Sched/Resched)", total_trial)

    # ---------- Time series ----------
    # choose metric(s)
    metric_opts = ["Deals","Enrollments","Calibration Done","Calibration Scheduled"]
    if (m_trial_sched.any() or m_trial_res.any() or m_trial_done.any()):
        metric_opts += ["Trials Scheduled","Trials Rescheduled","Trials Done"]
    chosen_metrics = st.multiselect("Metrics to plot", metric_opts, default=["Deals","Enrollments"])

    # base series date based on granularity
    if gran == "Day":
        freq = "D"
        tscol_for_deals = df["_create_dt"] if mode=="MTD" and df["_create_dt"].notna().any() else df["_pay_dt"]
    elif gran == "Week":
        freq = "W"
        tscol_for_deals = df["_create_dt"] if mode=="MTD" and df["_create_dt"].notna().any() else df["_pay_dt"]
    else:
    ...
    def series_count(ts, mask):
        s = ts[mask]
        if s.notna().any():
            g = s.dt.to_period(freq).dt.start_time.value_counts().sort_index()
            # Create a full index across window
            idx = pd.period_range(start=start.normalize(), end=end.normalize(), freq=freq).to_timestamp()
    ...
    ts_data = {}
    if "Deals" in chosen_metrics:
        ts_data["Deals"] = series_count(tscol_for_deals, mask_deals)
    if "Enrollments" in chosen_metrics:
        ts_data["Enrollments"] = series_count(df["_pay_dt"], mask_enroll)
    if "Calibration Done" in chosen_metrics:
        ts_data["Calibration Done"] = series_count(df["_cal_done_dt"], m_cal_done)
    if "Calibration Scheduled" in chosen_metrics:
        # combine first + resched (OR)
        sched_mask = m_cal_scheduled
        ts_data["Calibration Scheduled"] = series_count(df["_cal_first_dt"].fillna(df["_cal_resched_dt"]), sched_mask)
    if "Trials Scheduled" in chosen_metrics and "_trial_sched_dt" in df:
        ts_data["Trials Scheduled"] = series_count(df["_trial_sched_dt"], m_trial_sched)
    if "Trials Rescheduled" in chosen_metrics and "_trial_resched_dt" in df:
        ts_data["Trials Rescheduled"] = series_count(df["_trial_resched_dt"], m_trial_res)
    if "Trials Done" in chosen_metrics and "_trial_done_dt" in df:
        ts_data["Trials Done"] = series_count(df["_trial_done_dt"], m_trial_done)
    ...
    # ---------- Split table(s) ----------
    metric_for_table = st.selectbox("Metric for split tables", ["Deals","Enrollments","Calibration Done","Calibration Scheduled"], key="sd_metric_table")
    if metric_for_table == "Deals":
        metric_mask = mask_deals
        ts_for_metric = tscol_for_deals
    elif metric_for_table == "Enrollments":
        metric_mask = mask_enroll
        ts_for_metric = df["_pay_dt"]
    elif metric_for_table == "Calibration Done":
        metric_mask = m_cal_done
        ts_for_metric = df["_cal_done_dt"]
    else:
        metric_mask = m_cal_scheduled
        ts_for_metric = df["_cal_first_dt"].fillna(df["_cal_resched_dt"])

    base = df[metric_mask].copy()
    base["_sibling"] = df.loc[metric_mask, sibling_col].fillna("Unknown")
    base["_refsrc"] = df.loc[metric_mask, referral_col].fillna("Unknown")
    base["_ac"] = df.loc[metric_mask, ac_col] if ac_col else "N/A"
    base["_country"] = df.loc[metric_mask, country_col] if country_col else "N/A"

    # crosstab orientation
    if split_orientation == "Sibling Deal stacked by Referral":
        pt = pd.crosstab(base["_sibling"], base["_refsrc"])
    else:
        pt = pd.crosstab(base["_refsrc"], base["_sibling"])

    st.markdown("**Split table**")
    st.dataframe(pt)

    # AC-wise / Country-wise pivots
    if "AC-wise" in view_scope and ac_col:
        st.markdown("**AC-wise split**")
        if split_orientation == "Sibling Deal stacked by Referral":
            pt_ac = pd.pivot_table(base, values="_sibling", index="_ac", columns="_refsrc", aggfunc="count", fill_value=0)
        else:
            pt_ac = pd.pivot_table(base, values="_sibling", index="_ac", columns="_sibling", aggfunc="count", fill_value=0)
        st.dataframe(pt_ac)

    if "Country-wise" in view_scope and country_col:
        st.markdown("**Country-wise split**")
        if split_orientation == "Sibling Deal stacked by Referral":
            pt_cty = pd.pivot_table(base, values="_sibling", index="_country", columns="_refsrc", aggfunc="count", fill_value=0)
        else:
            pt_cty = pd.pivot_table(base, values="_sibling", index="_country", columns="_sibling", aggfunc="count", fill_value=0)
        st.dataframe(pt_cty)

    # CSV download
    def _download_df(df_in, label):
        import io
        csv = df_in.to_csv(index=True).encode("utf-8")
        st.download_button(f"Download CSV — {label}", data=csv, file_name=f"sibling_detail_{label.lower().replace(' ','_')}.csv", mime="text/csv")

    _download_df(pt, "split")
    if "AC-wise" in view_scope and ac_col:
        if split_orientation == "Sibling Deal stacked by Referral":
            pt_ac = pd.pivot_table(base, values="_sibling", index="_ac", columns="_refsrc", aggfunc="count", fill_value=0)
        else:
            pt_ac = pd.pivot_table(base, values="_sibling", index="_ac", columns="_sibling", aggfunc="count", fill_value=0)
        _download_df(pt_ac, "ac_wise")
    if "Country-wise" in view_scope and country_col:
        if split_orientation == "Sibling Deal stacked by Referral":
            pt_cty = pd.pivot_table(base, values="_sibling", index="_country", columns="_refsrc", aggfunc="count", fill_value=0)
        else:
            pt_cty = pd.pivot_table(base, values="_sibling", index="_country", columns="_sibling", aggfunc="count", fill_value=0)
        _download_df(pt_cty, "country_wise")
