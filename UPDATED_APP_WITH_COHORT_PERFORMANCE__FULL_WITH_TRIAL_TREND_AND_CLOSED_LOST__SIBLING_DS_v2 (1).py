...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time" , "Sibling DS"],
    }
...
def _render_marketing_sibling_ds(
    ...
    if ev_col is not None and base.shape[0] > 0:
        ...
        if gran == "Day":
            ...
            grp = base.copy()
            grp["Date"] = base[ev_col].dt.to_period("W").apply(lambda p: p.start_time.date())
            grp = grp.groupby([sibling_col, "Date"]).size().reset_index(name="Count")
        else:
            grp = base.copy()
            grp["Date"] = base[ev_col].dt.to_period("M").apply(lambda p: p.start_time.date())
            grp = grp.groupby([sibling_col, "Date"]).size().reset_index(name="Count")
